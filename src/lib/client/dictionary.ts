import { appConfig } from "@/lib/app-config";

export type Language = "en" | "es" | "fr" | "hi" | "mr";

export const languages: Record<Language, string> = {
  en: "English",
  es: "Español",
  fr: "Français",
  hi: "हिन्दी",
  mr: "मराठी"
};

export const dictionary = {
  en: {
    appName: appConfig.name,
    navigation: "Primary navigation",
    menu: "Menu",
    dashboard: "Dashboard",
    expenses: "Expenses",
    budgets: "Budgets",
    recurring: "Recurring",
    parties: "Parties",
    importReview: "Import review",
    reports: "Reports",
    support: "Support",
    settings: "Settings",
    account: "Account",
    theme: "Theme",
    language: "Language",
    light: "Light",
    dark: "Dark",
    online: "Online",
    offline: "Offline",
    offlineMessage: "You are offline. Changes are saved locally and can sync when service APIs are connected.",
    addExpense: "Add expense",
    export: "Export",
    search: "Search",
    category: "Category",
    amount: "Amount",
    date: "Date",
    status: "Status",
    merchant: "Merchant",
    owner: "Owner",
    progress: "Progress",
    totalSpend: "Total spend",
    remainingBudget: "Remaining budget",
    monthlyRemainingBudget: "Monthly remaining",
    yearlyRemainingBudget: "Yearly remaining",
    monthlyRunway: "Monthly runway",
    pendingImports: "Pending imports",
    investments: "Investments",
    save: "Save",
    syncNow: "Sync now",
    logout: "Log out",
    hi: "Hi",
    hello: "Hello",
    user: "User"
  },
  es: {
    appName: appConfig.name,
    navigation: "Navegación principal",
    menu: "Menú",
    dashboard: "Panel",
    expenses: "Gastos",
    budgets: "Presupuestos",
    recurring: "Recurrentes",
    parties: "Grupos",
    importReview: "Revisión de importación",
    reports: "Informes",
    support: "Soporte",
    settings: "Ajustes",
    account: "Cuenta",
    theme: "Tema",
    language: "Idioma",
    light: "Claro",
    dark: "Oscuro",
    online: "En línea",
    offline: "Sin conexión",
    offlineMessage: "Estás sin conexión. Los cambios se guardan localmente y se pueden sincronizar cuando las APIs estén conectadas.",
    addExpense: "Agregar gasto",
    export: "Exportar",
    search: "Buscar",
    category: "Categoría",
    amount: "Importe",
    date: "Fecha",
    status: "Estado",
    merchant: "Comercio",
    owner: "Responsable",
    progress: "Progreso",
    totalSpend: "Gasto total",
    remainingBudget: "Presupuesto restante",
    monthlyRemainingBudget: "Restante mensual",
    yearlyRemainingBudget: "Restante anual",
    monthlyRunway: "Margen mensual",
    pendingImports: "Importaciones pendientes",
    investments: "Inversiones",
    save: "Guardar",
    syncNow: "Sincronizar",
    logout: "Cerrar sesión",
    hi: "Hola",
    hello: "Hola",
    user: "Usuario"
  },
  fr: {
    appName: appConfig.name,
    navigation: "Navigation principale",
    menu: "Menu",
    dashboard: "Tableau de bord",
    expenses: "Dépenses",
    budgets: "Budgets",
    recurring: "Récurrents",
    parties: "Groupes",
    importReview: "Revue d'import",
    reports: "Rapports",
    support: "Support",
    settings: "Paramètres",
    account: "Compte",
    theme: "Thème",
    language: "Langue",
    light: "Clair",
    dark: "Sombre",
    online: "En ligne",
    offline: "Hors ligne",
    offlineMessage: "Vous êtes hors ligne. Les changements sont enregistrés localement et pourront se synchroniser lorsque les APIs seront connectées.",
    addExpense: "Ajouter une dépense",
    export: "Exporter",
    search: "Rechercher",
    category: "Catégorie",
    amount: "Montant",
    date: "Date",
    status: "Statut",
    merchant: "Marchand",
    owner: "Responsable",
    progress: "Progression",
    totalSpend: "Dépense totale",
    remainingBudget: "Budget restant",
    monthlyRemainingBudget: "Mensuel restant",
    yearlyRemainingBudget: "Annuel restant",
    monthlyRunway: "Autonomie mensuelle",
    pendingImports: "Imports en attente",
    investments: "Investissements",
    save: "Enregistrer",
    syncNow: "Synchroniser",
    logout: "Se déconnecter",
    hi: "Bonjour",
    hello: "Bonjour",
    user: "Utilisateur"
  },
  hi: {
    appName: appConfig.name,
    navigation: "मुख्य नेविगेशन",
    menu: "मेनू",
    dashboard: "डैशबोर्ड",
    expenses: "खर्च",
    budgets: "बजट",
    recurring: "आवर्ती",
    parties: "समूह",
    importReview: "आयात समीक्षा",
    reports: "रिपोर्ट",
    support: "सहायता",
    settings: "सेटिंग्स",
    account: "खाता",
    theme: "थीम",
    language: "भाषा",
    light: "लाइट",
    dark: "डार्क",
    online: "ऑनलाइन",
    offline: "ऑफलाइन",
    offlineMessage: "आप ऑफलाइन हैं। बदलाव स्थानीय रूप से सहेजे गए हैं और APIs कनेक्ट होने पर सिंक किए जा सकते हैं।",
    addExpense: "खर्च जोड़ें",
    export: "निर्यात",
    search: "खोजें",
    category: "श्रेणी",
    amount: "राशि",
    date: "तारीख",
    status: "स्थिति",
    merchant: "व्यापारी",
    owner: "मालिक",
    progress: "प्रगति",
    totalSpend: "कुल खर्च",
    remainingBudget: "शेष बजट",
    monthlyRemainingBudget: "मासिक शेष",
    yearlyRemainingBudget: "वार्षिक शेष",
    monthlyRunway: "मासिक रनवे",
    pendingImports: "लंबित आयात",
    investments: "निवेश",
    save: "सहेजें",
    syncNow: "अभी सिंक करें",
    logout: "लॉग आउट",
    hi: "नमस्ते",
    hello: "नमस्ते",
    user: "उपयोगकर्ता"
  },
  mr: {
    appName: appConfig.name,
    navigation: "मुख्य नेव्हिगेशन",
    menu: "मेनू",
    dashboard: "डॅशबोर्ड",
    expenses: "खर्च",
    budgets: "बजेट",
    recurring: "आवर्ती",
    parties: "समूह",
    importReview: "आयात पुनरावलोकन",
    reports: "अहवाल",
    support: "सहाय्य",
    settings: "सेटिंग्ज",
    account: "खाते",
    theme: "थीम",
    language: "भाषा",
    light: "लाइट",
    dark: "डार्क",
    online: "ऑनलाइन",
    offline: "ऑफलाइन",
    offlineMessage: "तुम्ही ऑफलाइन आहात. बदल स्थानिकरीत्या साठवले आहेत आणि APIs जोडल्यावर सिंक होऊ शकतात.",
    addExpense: "खर्च जोडा",
    export: "निर्यात",
    search: "शोधा",
    category: "श्रेणी",
    amount: "रक्कम",
    date: "तारीख",
    status: "स्थिती",
    merchant: "व्यापारी",
    owner: "मालक",
    progress: "प्रगती",
    totalSpend: "एकूण खर्च",
    remainingBudget: "उर्वरित बजेट",
    monthlyRemainingBudget: "मासिक उर्वरित",
    yearlyRemainingBudget: "वार्षिक उर्वरित",
    monthlyRunway: "मासिक रनवे",
    pendingImports: "प्रलंबित आयात",
    investments: "गुंतवणूक",
    save: "जतन करा",
    syncNow: "आता सिंक करा",
    logout: "लॉग आउट",
    hi: "नमस्कार",
    hello: "नमस्कार",
    user: "वापरकर्ता"
  }
} satisfies Record<Language, Record<string, string>>;

export const phraseDictionary: Record<Language, Record<string, string>> = {
  en: {},
  es: {
    Workspace: "Espacio de trabajo",
    Loading: "Cargando",
    Saving: "Guardando",
    Syncing: "Sincronizando",
    Delete: "Eliminar",
    Cancel: "Cancelar",
    Approve: "Aprobar",
    Reject: "Rechazar",
    Locked: "Bloqueado",
    Settled: "Liquidado",
    "Admin only": "Solo administrador",
    registered: "registrado",
    placeholder: "temporal",
    open: "abierto",
    "approval pending": "aprobación pendiente",
    "A calm command center for personal, party, and imported spending.": "Un centro de control tranquilo para gastos personales, de grupo e importados.",
    "Search, add, and review every transaction before it hits reports.": "Busca, agrega y revisa cada transacción antes de que llegue a los informes.",
    "Track limits, spot overspend early, and keep categories honest.": "Controla límites, detecta excesos temprano y mantén las categorías claras.",
    "Triage low-confidence rows before posting them into the ledger.": "Revisa filas de baja confianza antes de publicarlas en el libro.",
    "Track shared expenses, external participants, and settlements that need approval.": "Controla gastos compartidos, participantes externos y liquidaciones que necesitan aprobación.",
    "NovaCent reporting for category mix, cash flow, budget variance, merchants, parties, and currencies.": "Informes de NovaCent para mezcla de categorías, flujo de caja, variación de presupuesto, comercios, grupos y monedas.",
    "Workspace preferences, sync posture, and localization readiness.": "Preferencias del espacio de trabajo, estado de sincronización y preparación de localización.",
    "Across visible transactions this month": "En transacciones visibles de este mes",
    "Available across active envelopes": "Disponible en sobres activos",
    "Available in monthly budgets": "Disponible en presupuestos mensuales",
    "Available in yearly budgets": "Disponible en presupuestos anuales",
    "At the current seven-day average": "Según el promedio actual de siete días",
    "Rows waiting for review": "Filas esperando revisión",
    "Key account metrics": "Métricas clave de la cuenta",
    "Budget health": "Salud del presupuesto",
    "Recent activity": "Actividad reciente",
    "Expense ledger": "Libro de gastos",
    "Expense ledger with merchant, category, owner, amount, and status": "Libro de gastos con comercio, categoría, responsable, importe y estado",
    "No expenses yet": "Aún no hay gastos",
    "New spending will appear here once imported or added.": "Los nuevos gastos aparecerán aquí cuando se importen o agreguen.",
    "Action": "Acción",
    "Quick add": "Agregar rápido",
    "Expense saved or queued for sync.": "Gasto guardado o en cola para sincronizar.",
    "Unable to save expense.": "No se pudo guardar el gasto.",
    "Expense deleted.": "Gasto eliminado.",
    "Unable to delete expense. Settled party expenses and settlement rows are locked.": "No se pudo eliminar el gasto. Los gastos de grupo liquidados y las filas de liquidación están bloqueados.",
    "Merchant, category, owner": "Comercio, categoría, responsable",
    Merchant: "Comercio",
    Category: "Categoría",
    Amount: "Importe",
    Currency: "Moneda",
    Date: "Fecha",
    Owner: "Responsable",
    Status: "Estado",
    Food: "Comida",
    Groceries: "Comestibles",
    Dining: "Restaurantes",
    Shopping: "Compras",
    Travel: "Viajes",
    "Bike Expense": "Gastos de moto",
    Transport: "Transporte",
    Utilities: "Servicios",
    Rent: "Alquiler",
    Education: "Educación",
    Entertainment: "Entretenimiento",
    "Personal Care": "Cuidado personal",
    Household: "Hogar",
    Insurance: "Seguro",
    Taxes: "Impuestos",
    Gifts: "Regalos",
    Subscriptions: "Suscripciones",
    Health: "Salud",
    Fuel: "Combustible",
    "Loan/EMI": "Prestamo/EMI",
    Investments: "Inversiones",
    Others: "Otros",
    "New budget": "Nuevo presupuesto",
    "Edit budget": "Editar presupuesto",
    "Budget name or category": "Nombre o categoría del presupuesto",
    Tracks: "Controla",
    Limit: "Límite",
    Period: "Periodo",
    "Alert at %": "Alerta en %",
    "Overall spend": "Gasto total",
    "Single category": "Una categoría",
    Monthly: "Mensual",
    Yearly: "Anual",
    "Save budget": "Guardar presupuesto",
    "Update budget": "Actualizar presupuesto",
    "Budget saved.": "Presupuesto guardado.",
    "Unable to save budget.": "No se pudo guardar el presupuesto.",
    "Budget deleted.": "Presupuesto eliminado.",
    "Unable to delete budget.": "No se pudo eliminar el presupuesto.",
    "Active budgets": "Presupuestos activos",
    "this month": "este mes",
    "this year": "este año",
    month: "mes",
    year: "año",
    of: "de",
    spend: "gasto",
    "Jan 1 to Dec 31": "1 de enero a 31 de diciembre",
    "1st of this month to month end": "Del día 1 de este mes al fin de mes",
    "Import review": "Revisión de importación",
    "Upload statement": "Subir extracto",
    "Uploading and parsing statement.": "Subiendo y analizando extracto.",
    "Statement import failed.": "Falló la importación del extracto.",
    "Statement rows are ready for review.": "Las filas del extracto están listas para revisión.",
    "Rows needing attention": "Filas que necesitan atención",
    "Import row filter": "Filtro de filas importadas",
    All: "Todo",
    "Possible duplicates": "Posibles duplicados",
    "Imported transaction review queue": "Cola de revisión de transacciones importadas",
    Source: "Origen",
    "Suggested category": "Categoría sugerida",
    Confidence: "Confianza",
    "possible duplicate": "posible duplicado",
    "No rows match this filter": "Ninguna fila coincide con este filtro",
    "Switch back to all rows to continue import review.": "Vuelve a todas las filas para continuar la revisión.",
    Parties: "Grupos",
    "Back to parties": "Volver a grupos",
    "Create party": "Crear grupo",
    "Party deleted.": "Grupo eliminado.",
    "Unable to delete party. Parties with settled expenses are locked.": "No se pudo eliminar el grupo. Los grupos con gastos liquidados están bloqueados.",
    "Party name": "Nombre del grupo",
    "Weekend friends": "Amigos de fin de semana",
    "Save party": "Guardar grupo",
    "Unable to save party.": "No se pudo guardar el grupo.",
    "Party saved.": "Grupo guardado.",
    "Group balances": "Saldos de grupo",
    "Open party": "Abrir grupo",
    "No parties yet": "Aún no hay grupos",
    "Create a party to track shared expenses and settlements.": "Crea un grupo para controlar gastos compartidos y liquidaciones.",
    "Loading party workspace": "Cargando espacio del grupo",
    "Unable to load party details.": "No se pudieron cargar los detalles del grupo.",
    workspace: "espacio de trabajo",
    "Delete party": "Eliminar grupo",
    Participants: "Participantes",
    "Search friend": "Buscar amigo",
    "Name or email": "Nombre o email",
    Search: "Buscar",
    Add: "Agregar",
    "as non-registered person": "como persona no registrada",
    "Participant list updated.": "Lista de participantes actualizada.",
    "Unable to update participant list.": "No se pudo actualizar la lista de participantes.",
    "Add party expense": "Agregar gasto de grupo",
    "Paid by": "Pagado por",
    "Split mode": "Modo de división",
    "Even split": "División equitativa",
    "Manual amounts": "Importes manuales",
    owes: "debe",
    "Save party expense": "Guardar gasto de grupo",
    "Party expense and splits saved.": "Gasto de grupo y divisiones guardados.",
    "Unable to save party expense or splits.": "No se pudo guardar el gasto de grupo o sus divisiones.",
    "Party expenses": "Gastos de grupo",
    "Party expense deleted.": "Gasto de grupo eliminado.",
    "Unable to delete party expense. Settled party expenses are locked.": "No se pudo eliminar el gasto de grupo. Los gastos liquidados están bloqueados.",
    "Party expenses that feed the split ledger": "Gastos de grupo que alimentan el libro de divisiones",
    "No party expenses yet": "Aún no hay gastos de grupo",
    "Add a party expense to start tracking splits.": "Agrega un gasto de grupo para comenzar a controlar divisiones.",
    "Party settlement actions": "Acciones de liquidación de grupo",
    "Splits and settlements": "Divisiones y liquidaciones",
    "Party split ledger": "Libro de divisiones del grupo",
    Expense: "Gasto",
    "Owed by": "Debe",
    "Party expense": "Gasto de grupo",
    Unknown: "Desconocido",
    Participant: "Participante",
    "Mark settled": "Marcar liquidado",
    "Waiting approval": "Esperando aprobación",
    "No open splits": "No hay divisiones abiertas",
    "Add a party expense to generate split settlement actions.": "Agrega un gasto de grupo para generar acciones de liquidación.",
    "Approval requests": "Solicitudes de aprobación",
    "marked settled": "marcó liquidado",
    "needs approval from": "necesita aprobación de",
    "the payer": "el pagador",
    "before the split closes.": "antes de cerrar la división.",
    "Waiting for": "Esperando a",
    "to review.": "para revisar.",
    "External placeholder settled directly.": "El participante temporal se liquidó directamente.",
    "Settlement approval request created.": "Solicitud de aprobación de liquidación creada.",
    "Unable to mark this split as settled.": "No se pudo marcar esta división como liquidada.",
    "Settlement approved.": "Liquidación aprobada.",
    "Settlement rejected.": "Liquidación rechazada.",
    "Unable to review settlement.": "No se pudo revisar la liquidación.",
    Reports: "Informes",
    "Export CSV": "Exportar CSV",
    "Export PDF": "Exportar PDF",
    "Loading NovaCent report charts": "Cargando gráficos de informes de NovaCent",
    "Report summary metrics": "Métricas resumen del informe",
    "Tracked spend": "Gasto registrado",
    "Across active report categories": "En categorías activas del informe",
    "Cash retained": "Efectivo retenido",
    "Income minus spend in visible months": "Ingresos menos gastos en meses visibles",
    "Largest budget usage": "Mayor uso de presupuesto",
    "Highest active category utilization": "Mayor uso de categoría activa",
    Currencies: "Monedas",
    "Original spend currencies represented": "Monedas originales de gasto representadas",
    "Spend by category": "Gasto por categoría",
    "Cash flow trend": "Tendencia de flujo de caja",
    "Budget variance": "Variación de presupuesto",
    "Merchant trends": "Tendencias de comercios",
    "Parties and currencies": "Grupos y monedas",
    "Currency exposure": "Exposición de moneda",
    "Spend by original currency chart": "Gráfico de gasto por moneda original",
    Preferences: "Preferencias",
    "Default currency": "Moneda predeterminada",
    "Preferences saved on this device.": "Preferencias guardadas en este dispositivo.",
    "Offline readiness": "Preparación sin conexión",
    "Local preferences persist in browser storage.": "Las preferencias locales persisten en el almacenamiento del navegador.",
    "Live APIs are used by default; set NEXT_PUBLIC_USE_MOCKS=true for demo data.": "Las APIs en vivo se usan por defecto; establece NEXT_PUBLIC_USE_MOCKS=true para datos demo.",
    "Legacy app-shell caches are cleared in live mode so old demo screens do not keep rendering after refresh.": "Las cachés heredadas del shell se limpian en modo en vivo para que pantallas demo antiguas no sigan apareciendo tras refrescar.",
    "Offline banner reacts to browser connectivity events.": "El aviso offline responde a eventos de conectividad del navegador.",
    "Sign in to": "Iniciar sesión en",
    "Use email/password or Google to open your account workspace.": "Usa email/contraseña o Google para abrir tu espacio de cuenta.",
    Email: "Email",
    Password: "Contraseña",
    "Signing in": "Iniciando sesión",
    "Sign in": "Iniciar sesión",
    "Continue with Google": "Continuar con Google",
    "Invalid email or password.": "Email o contraseña inválidos.",
    "Create an account": "Crear una cuenta",
    "Forgot password?": "¿Olvidaste la contraseña?",
    "Create your account": "Crea tu cuenta",
    "A default INR account is created automatically after registration.": "Se crea automáticamente una cuenta INR predeterminada después del registro.",
    Name: "Nombre",
    "Creating account": "Creando cuenta",
    "Create account": "Crear cuenta",
    "Unable to create account. The email may already be registered.": "No se pudo crear la cuenta. El email quizá ya esté registrado.",
    "Already have an account?": "¿Ya tienes una cuenta?",
    "Reset password": "Restablecer contraseña",
    "We prepare a secure single-use reset token that expires after 30 minutes.": "Preparamos un token seguro de un solo uso que expira después de 30 minutos.",
    "Preparing link": "Preparando enlace",
    "Send reset link": "Enviar enlace de restablecimiento",
    "If the email exists, a reset link has been prepared.": "Si el email existe, se preparó un enlace de restablecimiento.",
    "Open development reset link": "Abrir enlace de restablecimiento de desarrollo",
    "Back to sign in": "Volver a iniciar sesión",
    "Choose a new password": "Elige una nueva contraseña",
    "Use at least 8 characters. The reset token can only be used once.": "Usa al menos 8 caracteres. El token solo puede usarse una vez.",
    "New password": "Nueva contraseña",
    "Updating password": "Actualizando contraseña",
    "Update password": "Actualizar contraseña",
    "Reset token is invalid or expired.": "El token es inválido o expiró.",
    "Data table": "Tabla de datos",
    "Category mix": "Mezcla de categorías",
    "Category spend totals": "Totales de gasto por categoría",
    Spend: "Gasto",
    Month: "Mes",
    Income: "Ingresos",
    Saved: "Ahorrado",
    Actual: "Real",
    Remaining: "Restante",
    Used: "Usado",
    Label: "Etiqueta",
    Outstanding: "Pendiente",
    Notifications: "Notificaciones",
    unread: "sin leer",
    "Mark read": "Marcar leídas",
    "No notifications yet.": "Aún no hay notificaciones.",
    "Close navigation": "Cerrar navegación",
    "Open navigation": "Abrir navegación",
    "Loading account": "Cargando cuenta",
    "Loading dashboard": "Cargando panel",
    "Sync complete.": "Sincronización completa.",
    "Unable to sync right now.": "No se puede sincronizar ahora.",
    days: "días",
    cleared: "liquidado",
    pending: "pendiente",
    failed: "fallido",
    "Monthly Budget": "Presupuesto mensual",
    "Simple category bar summary": "Resumen simple de barras por categoría",
    "Budget": "Presupuesto",
    "Food merchants": "Comercios de comida",
    "Travel merchants": "Comercios de viaje",
    "Shopping merchants": "Comercios de compras",
    "Monthly cash flow": "Flujo de caja mensual",
    "Merchant trend lanes": "Carriles de tendencia de comercios",
    "Party balances": "Saldos de grupos",
    "Monthly income, spend, and saved amount trend chart": "Gráfico mensual de ingresos, gastos y ahorro",
    "Budget variance chart comparing actual spend with budget limit": "Gráfico de variación entre gasto real y límite de presupuesto",
    "Merchant and category spending trends by month": "Tendencias mensuales de gasto por comercio y categoría",
    "Party outstanding and settled balance comparison": "Comparación de saldos pendientes y liquidados de grupos",
    "Monthly income, spend, and saved report": "Informe mensual de ingresos, gastos y ahorro",
    "Budget variance by category": "Variación de presupuesto por categoría",
    "Party outstanding and settled balances": "Saldos pendientes y liquidados de grupos",
    Party: "Grupo",
    "Category spend chart with": "Gráfico de gasto por categoría con",
    home: "inicio",
    "bar chart": "gráfico de barras",
    "from expenses? This cannot be undone.": "de gastos? Esto no se puede deshacer.",
    "budget?": "presupuesto?",
    "and all unlocked party expenses?": "y todos los gastos de grupo desbloqueados?",
    "from this party and the overall expense ledger?": "de este grupo y del libro general de gastos?",
    "match confidence": "confianza de coincidencia",
    "Party participants": "Participantes del grupo",
    data: "datos"
  },
  fr: {
    Workspace: "Espace de travail",
    Loading: "Chargement",
    Saving: "Enregistrement",
    Syncing: "Synchronisation",
    Delete: "Supprimer",
    Cancel: "Annuler",
    Approve: "Approuver",
    Reject: "Rejeter",
    Locked: "Verrouillé",
    Settled: "Réglé",
    "Admin only": "Admin seulement",
    registered: "inscrit",
    placeholder: "temporaire",
    open: "ouvert",
    "approval pending": "approbation en attente",
    "A calm command center for personal, party, and imported spending.": "Un centre de contrôle clair pour les dépenses personnelles, de groupe et importées.",
    "Search, add, and review every transaction before it hits reports.": "Recherchez, ajoutez et vérifiez chaque transaction avant qu'elle arrive dans les rapports.",
    "Track limits, spot overspend early, and keep categories honest.": "Suivez les limites, repérez vite les dépassements et gardez les catégories fiables.",
    "Triage low-confidence rows before posting them into the ledger.": "Vérifiez les lignes peu fiables avant de les publier dans le registre.",
    "Track shared expenses, external participants, and settlements that need approval.": "Suivez les dépenses partagées, les participants externes et les règlements à approuver.",
    "NovaCent reporting for category mix, cash flow, budget variance, merchants, parties, and currencies.": "Rapports NovaCent pour les catégories, le flux de trésorerie, les écarts de budget, les marchands, les groupes et les devises.",
    "Workspace preferences, sync posture, and localization readiness.": "Préférences de l'espace, état de synchronisation et préparation linguistique.",
    "Across visible transactions this month": "Sur les transactions visibles ce mois-ci",
    "Available across active envelopes": "Disponible dans les enveloppes actives",
    "Available in monthly budgets": "Disponible dans les budgets mensuels",
    "Available in yearly budgets": "Disponible dans les budgets annuels",
    "At the current seven-day average": "Au rythme moyen des sept derniers jours",
    "Rows waiting for review": "Lignes en attente de revue",
    "Key account metrics": "Indicateurs clés du compte",
    "Budget health": "Santé du budget",
    "Recent activity": "Activité récente",
    "Expense ledger": "Registre des dépenses",
    "Expense ledger with merchant, category, owner, amount, and status": "Registre avec marchand, catégorie, responsable, montant et statut",
    "No expenses yet": "Aucune dépense pour l'instant",
    "New spending will appear here once imported or added.": "Les nouvelles dépenses apparaîtront ici après import ou ajout.",
    Action: "Action",
    "Quick add": "Ajout rapide",
    "Expense saved or queued for sync.": "Dépense enregistrée ou mise en file de synchronisation.",
    "Unable to save expense.": "Impossible d'enregistrer la dépense.",
    "Expense deleted.": "Dépense supprimée.",
    "Unable to delete expense. Settled party expenses and settlement rows are locked.": "Impossible de supprimer la dépense. Les dépenses de groupe réglées et les lignes de règlement sont verrouillées.",
    "Merchant, category, owner": "Marchand, catégorie, responsable",
    Merchant: "Marchand",
    Category: "Catégorie",
    Amount: "Montant",
    Currency: "Devise",
    Date: "Date",
    Owner: "Responsable",
    Status: "Statut",
    Food: "Alimentation",
    Groceries: "Courses",
    Dining: "Restaurants",
    Shopping: "Achats",
    Travel: "Voyage",
    "Bike Expense": "Frais moto",
    Transport: "Transport",
    Utilities: "Services",
    Rent: "Loyer",
    Education: "Éducation",
    Entertainment: "Divertissement",
    "Personal Care": "Soins personnels",
    Household: "Maison",
    Insurance: "Assurance",
    Taxes: "Impôts",
    Gifts: "Cadeaux",
    Subscriptions: "Abonnements",
    Health: "Santé",
    Fuel: "Carburant",
    "Loan/EMI": "Pret/EMI",
    Investments: "Investissements",
    Others: "Autres",
    "New budget": "Nouveau budget",
    "Edit budget": "Modifier le budget",
    "Budget name or category": "Nom ou catégorie du budget",
    Tracks: "Suit",
    Limit: "Limite",
    Period: "Période",
    "Alert at %": "Alerte à %",
    "Overall spend": "Dépense totale",
    "Single category": "Catégorie unique",
    Monthly: "Mensuel",
    Yearly: "Annuel",
    "Save budget": "Enregistrer le budget",
    "Update budget": "Mettre à jour le budget",
    "Budget saved.": "Budget enregistré.",
    "Unable to save budget.": "Impossible d'enregistrer le budget.",
    "Budget deleted.": "Budget supprimé.",
    "Unable to delete budget.": "Impossible de supprimer le budget.",
    "Active budgets": "Budgets actifs",
    "this month": "ce mois-ci",
    "this year": "cette année",
    month: "mois",
    year: "année",
    of: "sur",
    spend: "dépense",
    "Jan 1 to Dec 31": "1 janv. au 31 déc.",
    "1st of this month to month end": "Du 1er du mois à la fin du mois",
    "Import review": "Revue d'import",
    "Upload statement": "Importer un relevé",
    "Uploading and parsing statement.": "Import et analyse du relevé.",
    "Statement import failed.": "Échec de l'import du relevé.",
    "Statement rows are ready for review.": "Les lignes du relevé sont prêtes à être vérifiées.",
    "Rows needing attention": "Lignes à vérifier",
    "Import row filter": "Filtre des lignes importées",
    All: "Tout",
    "Possible duplicates": "Doublons possibles",
    "Imported transaction review queue": "File de revue des transactions importées",
    Source: "Source",
    "Suggested category": "Catégorie suggérée",
    Confidence: "Confiance",
    "possible duplicate": "doublon possible",
    "No rows match this filter": "Aucune ligne ne correspond à ce filtre",
    "Switch back to all rows to continue import review.": "Revenez à toutes les lignes pour continuer la revue.",
    Parties: "Groupes",
    "Back to parties": "Retour aux groupes",
    "Create party": "Créer un groupe",
    "Party deleted.": "Groupe supprimé.",
    "Unable to delete party. Parties with settled expenses are locked.": "Impossible de supprimer le groupe. Les groupes avec dépenses réglées sont verrouillés.",
    "Party name": "Nom du groupe",
    "Weekend friends": "Amis du week-end",
    "Save party": "Enregistrer le groupe",
    "Unable to save party.": "Impossible d'enregistrer le groupe.",
    "Party saved.": "Groupe enregistré.",
    "Group balances": "Soldes du groupe",
    "Open party": "Ouvrir le groupe",
    "No parties yet": "Aucun groupe pour l'instant",
    "Create a party to track shared expenses and settlements.": "Créez un groupe pour suivre les dépenses partagées et les règlements.",
    "Loading party workspace": "Chargement de l'espace du groupe",
    "Unable to load party details.": "Impossible de charger les détails du groupe.",
    workspace: "espace",
    "Delete party": "Supprimer le groupe",
    Participants: "Participants",
    "Search friend": "Rechercher un ami",
    "Name or email": "Nom ou email",
    Search: "Rechercher",
    Add: "Ajouter",
    "as non-registered person": "comme personne non inscrite",
    "Participant list updated.": "Liste des participants mise à jour.",
    "Unable to update participant list.": "Impossible de mettre à jour la liste des participants.",
    "Add party expense": "Ajouter une dépense de groupe",
    "Paid by": "Payé par",
    "Split mode": "Mode de partage",
    "Even split": "Partage égal",
    "Manual amounts": "Montants manuels",
    owes: "doit",
    "Save party expense": "Enregistrer la dépense de groupe",
    "Party expense and splits saved.": "Dépense de groupe et partages enregistrés.",
    "Unable to save party expense or splits.": "Impossible d'enregistrer la dépense ou les partages.",
    "Party expenses": "Dépenses de groupe",
    "Party expense deleted.": "Dépense de groupe supprimée.",
    "Unable to delete party expense. Settled party expenses are locked.": "Impossible de supprimer la dépense de groupe. Les dépenses réglées sont verrouillées.",
    "Party expenses that feed the split ledger": "Dépenses de groupe qui alimentent le registre des partages",
    "No party expenses yet": "Aucune dépense de groupe pour l'instant",
    "Add a party expense to start tracking splits.": "Ajoutez une dépense de groupe pour commencer à suivre les partages.",
    "Party settlement actions": "Actions de règlement du groupe",
    "Splits and settlements": "Partages et règlements",
    "Party split ledger": "Registre des partages",
    Expense: "Dépense",
    "Owed by": "Dû par",
    "Party expense": "Dépense de groupe",
    Unknown: "Inconnu",
    Participant: "Participant",
    "Mark settled": "Marquer réglé",
    "Waiting approval": "En attente d'approbation",
    "No open splits": "Aucun partage ouvert",
    "Add a party expense to generate split settlement actions.": "Ajoutez une dépense de groupe pour générer les règlements.",
    "Approval requests": "Demandes d'approbation",
    "marked settled": "a marqué comme réglé",
    "needs approval from": "nécessite l'approbation de",
    "the payer": "le payeur",
    "before the split closes.": "avant la clôture du partage.",
    "Waiting for": "En attente de",
    "to review.": "pour vérification.",
    "External placeholder settled directly.": "Le participant temporaire a été réglé directement.",
    "Settlement approval request created.": "Demande d'approbation créée.",
    "Unable to mark this split as settled.": "Impossible de marquer ce partage comme réglé.",
    "Settlement approved.": "Règlement approuvé.",
    "Settlement rejected.": "Règlement rejeté.",
    "Unable to review settlement.": "Impossible de vérifier le règlement.",
    Reports: "Rapports",
    "Export CSV": "Exporter CSV",
    "Export PDF": "Exporter PDF",
    "Loading NovaCent report charts": "Chargement des graphiques NovaCent",
    "Report summary metrics": "Indicateurs du rapport",
    "Tracked spend": "Dépense suivie",
    "Across active report categories": "Dans les catégories actives du rapport",
    "Cash retained": "Trésorerie conservée",
    "Income minus spend in visible months": "Revenus moins dépenses sur les mois visibles",
    "Largest budget usage": "Plus forte utilisation du budget",
    "Highest active category utilization": "Plus forte utilisation d'une catégorie active",
    Currencies: "Devises",
    "Original spend currencies represented": "Devises originales représentées",
    "Spend by category": "Dépense par catégorie",
    "Cash flow trend": "Tendance du flux de trésorerie",
    "Budget variance": "Écart de budget",
    "Merchant trends": "Tendances des marchands",
    "Parties and currencies": "Groupes et devises",
    "Currency exposure": "Exposition aux devises",
    "Spend by original currency chart": "Graphique des dépenses par devise d'origine",
    Preferences: "Préférences",
    "Default currency": "Devise par défaut",
    "Preferences saved on this device.": "Préférences enregistrées sur cet appareil.",
    "Offline readiness": "Préparation hors ligne",
    "Local preferences persist in browser storage.": "Les préférences locales sont conservées dans le navigateur.",
    "Live APIs are used by default; set NEXT_PUBLIC_USE_MOCKS=true for demo data.": "Les APIs réelles sont utilisées par défaut; définissez NEXT_PUBLIC_USE_MOCKS=true pour les données de démonstration.",
    "Legacy app-shell caches are cleared in live mode so old demo screens do not keep rendering after refresh.": "Les anciens caches sont nettoyés en mode réel afin d'éviter les anciens écrans de démonstration après actualisation.",
    "Offline banner reacts to browser connectivity events.": "Le bandeau hors ligne réagit aux événements de connectivité.",
    "Sign in to": "Se connecter à",
    "Use email/password or Google to open your account workspace.": "Utilisez email/mot de passe ou Google pour ouvrir votre espace de compte.",
    Email: "Email",
    Password: "Mot de passe",
    "Signing in": "Connexion",
    "Sign in": "Se connecter",
    "Continue with Google": "Continuer avec Google",
    "Invalid email or password.": "Email ou mot de passe invalide.",
    "Create an account": "Créer un compte",
    "Forgot password?": "Mot de passe oublié ?",
    "Create your account": "Créer votre compte",
    "A default INR account is created automatically after registration.": "Un compte INR par défaut est créé automatiquement après l'inscription.",
    Name: "Nom",
    "Creating account": "Création du compte",
    "Create account": "Créer le compte",
    "Unable to create account. The email may already be registered.": "Impossible de créer le compte. L'email est peut-être déjà inscrit.",
    "Already have an account?": "Vous avez déjà un compte ?",
    "Reset password": "Réinitialiser le mot de passe",
    "We prepare a secure single-use reset token that expires after 30 minutes.": "Nous préparons un jeton sécurisé à usage unique qui expire après 30 minutes.",
    "Preparing link": "Préparation du lien",
    "Send reset link": "Envoyer le lien",
    "If the email exists, a reset link has been prepared.": "Si l'email existe, un lien de réinitialisation a été préparé.",
    "Open development reset link": "Ouvrir le lien de développement",
    "Back to sign in": "Retour à la connexion",
    "Choose a new password": "Choisir un nouveau mot de passe",
    "Use at least 8 characters. The reset token can only be used once.": "Utilisez au moins 8 caractères. Le jeton ne peut être utilisé qu'une fois.",
    "New password": "Nouveau mot de passe",
    "Updating password": "Mise à jour du mot de passe",
    "Update password": "Mettre à jour le mot de passe",
    "Reset token is invalid or expired.": "Le jeton est invalide ou expiré.",
    "Data table": "Table de données",
    "Category mix": "Répartition des catégories",
    "Category spend totals": "Totaux par catégorie",
    Spend: "Dépense",
    Month: "Mois",
    Income: "Revenus",
    Saved: "Économisé",
    Actual: "Réel",
    Remaining: "Restant",
    Used: "Utilisé",
    Label: "Libellé",
    Outstanding: "En attente",
    Notifications: "Notifications",
    unread: "non lues",
    "Mark read": "Marquer comme lu",
    "No notifications yet.": "Aucune notification pour l'instant.",
    "Close navigation": "Fermer la navigation",
    "Open navigation": "Ouvrir la navigation",
    "Loading account": "Chargement du compte",
    "Loading dashboard": "Chargement du tableau de bord",
    "Sync complete.": "Synchronisation terminée.",
    "Unable to sync right now.": "Impossible de synchroniser maintenant.",
    days: "jours",
    cleared: "validé",
    pending: "en attente",
    failed: "échoué",
    "Monthly Budget": "Budget mensuel",
    "Simple category bar summary": "Résumé simple par barres des catégories",
    "Budget": "Budget",
    "Food merchants": "Marchands alimentaires",
    "Travel merchants": "Marchands de voyage",
    "Shopping merchants": "Marchands d'achats",
    "Monthly cash flow": "Flux de trésorerie mensuel",
    "Merchant trend lanes": "Tendances par marchand",
    "Party balances": "Soldes des groupes",
    "Monthly income, spend, and saved amount trend chart": "Graphique mensuel des revenus, dépenses et économies",
    "Budget variance chart comparing actual spend with budget limit": "Graphique comparant la dépense réelle à la limite du budget",
    "Merchant and category spending trends by month": "Tendances mensuelles par marchand et catégorie",
    "Party outstanding and settled balance comparison": "Comparaison des soldes en attente et réglés",
    "Monthly income, spend, and saved report": "Rapport mensuel revenus, dépenses et économies",
    "Budget variance by category": "Écart de budget par catégorie",
    "Party outstanding and settled balances": "Soldes en attente et réglés des groupes",
    Party: "Groupe",
    "Category spend chart with": "Graphique des dépenses par catégorie avec",
    home: "accueil",
    "bar chart": "graphique en barres",
    "from expenses? This cannot be undone.": "des dépenses ? Cette action est irréversible.",
    "budget?": "budget ?",
    "and all unlocked party expenses?": "et toutes les dépenses de groupe déverrouillées ?",
    "from this party and the overall expense ledger?": "de ce groupe et du grand livre global ?",
    "match confidence": "confiance de correspondance",
    "Party participants": "Participants du groupe",
    data: "données"
  },
  hi: {
    Workspace: "कार्यक्षेत्र",
    Loading: "लोड हो रहा है",
    Saving: "सहेजा जा रहा है",
    Syncing: "सिंक हो रहा है",
    Delete: "हटाएं",
    Cancel: "रद्द करें",
    Approve: "स्वीकार करें",
    Reject: "अस्वीकार करें",
    Locked: "लॉक",
    Settled: "निपट गया",
    "Admin only": "केवल एडमिन",
    registered: "पंजीकृत",
    placeholder: "प्लेसहोल्डर",
    open: "खुला",
    "approval pending": "अनुमोदन लंबित",
    "A calm command center for personal, party, and imported spending.": "निजी, समूह और आयातित खर्चों के लिए शांत नियंत्रण केंद्र।",
    "Across visible transactions this month": "इस महीने दिखने वाले लेनदेन में",
    "Available across active envelopes": "सक्रिय बजट में उपलब्ध",
    "Available in monthly budgets": "मासिक बजट में उपलब्ध",
    "Available in yearly budgets": "वार्षिक बजट में उपलब्ध",
    "At the current seven-day average": "मौजूदा सात-दिन के औसत पर",
    "Rows waiting for review": "समीक्षा के लिए प्रतीक्षा कर रही पंक्तियां",
    "Budget health": "बजट स्वास्थ्य",
    "Recent activity": "हाल की गतिविधि",
    "Expense ledger": "खर्च लेजर",
    "Expense ledger with merchant, category, owner, amount, and status": "व्यापारी, श्रेणी, मालिक, राशि और स्थिति वाला खर्च लेजर",
    Action: "कार्रवाई",
    "Quick add": "त्वरित जोड़ें",
    Merchant: "व्यापारी",
    Category: "श्रेणी",
    Amount: "राशि",
    Currency: "मुद्रा",
    Date: "तारीख",
    Owner: "मालिक",
    Status: "स्थिति",
    Food: "भोजन",
    Groceries: "किराना",
    Dining: "बाहर खाना",
    Shopping: "खरीदारी",
    Travel: "यात्रा",
    "Bike Expense": "बाइक खर्च",
    Transport: "परिवहन",
    Utilities: "यूटिलिटी बिल",
    Rent: "किराया",
    Education: "शिक्षा",
    Entertainment: "मनोरंजन",
    "Personal Care": "व्यक्तिगत देखभाल",
    Household: "घर",
    Insurance: "बीमा",
    Taxes: "कर",
    Gifts: "उपहार",
    Subscriptions: "सदस्यताएं",
    Health: "स्वास्थ्य",
    Fuel: "ईंधन",
    "Loan/EMI": "लोन/ईएमआई",
    Investments: "निवेश",
    Others: "अन्य",
    "New budget": "नया बजट",
    "Edit budget": "बजट संपादित करें",
    "Active budgets": "सक्रिय बजट",
    "Overall spend": "कुल खर्च",
    "Single category": "एक श्रेणी",
    Monthly: "मासिक",
    Yearly: "वार्षिक",
    "this month": "इस महीने",
    "this year": "इस साल",
    month: "महीना",
    year: "साल",
    of: "में से",
    spend: "खर्च",
    "Import review": "आयात समीक्षा",
    "Upload statement": "स्टेटमेंट अपलोड करें",
    All: "सभी",
    "Possible duplicates": "संभावित डुप्लिकेट",
    Parties: "समूह",
    "Back to parties": "समूहों पर वापस",
    "Create party": "समूह बनाएं",
    "Group balances": "समूह बैलेंस",
    "Open party": "समूह खोलें",
    workspace: "कार्यक्षेत्र",
    Participants: "प्रतिभागी",
    Search: "खोजें",
    Add: "जोड़ें",
    "Add party expense": "समूह खर्च जोड़ें",
    "Paid by": "भुगतान किया",
    "Split mode": "विभाजन मोड",
    "Even split": "बराबर विभाजन",
    "Manual amounts": "मैनुअल राशि",
    owes: "देना है",
    "Party expenses": "समूह खर्च",
    "Splits and settlements": "विभाजन और निपटान",
    Expense: "खर्च",
    "Owed by": "देय",
    Unknown: "अज्ञात",
    Participant: "प्रतिभागी",
    "Mark settled": "निपट गया मार्क करें",
    "Waiting approval": "अनुमोदन की प्रतीक्षा",
    Reports: "रिपोर्ट",
    "Export CSV": "CSV निर्यात",
    "Export PDF": "PDF निर्यात",
    "Tracked spend": "ट्रैक किया खर्च",
    "Cash retained": "बची नकदी",
    "Largest budget usage": "सबसे बड़ा बजट उपयोग",
    Currencies: "मुद्राएं",
    "Spend by category": "श्रेणी के अनुसार खर्च",
    "Cash flow trend": "कैश फ्लो ट्रेंड",
    "Budget variance": "बजट अंतर",
    "Merchant trends": "व्यापारी ट्रेंड",
    "Parties and currencies": "समूह और मुद्राएं",
    Preferences: "प्राथमिकताएं",
    "Default currency": "डिफॉल्ट मुद्रा",
    "Offline readiness": "ऑफलाइन तैयारी",
    "Sign in to": "साइन इन करें",
    Email: "ईमेल",
    Password: "पासवर्ड",
    "Sign in": "साइन इन",
    "Continue with Google": "Google से जारी रखें",
    "Create an account": "खाता बनाएं",
    "Forgot password?": "पासवर्ड भूल गए?",
    Name: "नाम",
    "Create account": "खाता बनाएं",
    "Reset password": "पासवर्ड रीसेट करें",
    "New password": "नया पासवर्ड",
    "Update password": "पासवर्ड अपडेट करें",
    "Data table": "डेटा तालिका",
    "Category mix": "श्रेणी मिश्रण",
    Spend: "खर्च",
    Month: "महीना",
    Income: "आय",
    Saved: "बचत",
    Actual: "वास्तविक",
    Remaining: "शेष",
    Used: "उपयोग",
    Label: "लेबल",
    Outstanding: "बकाया",
    Notifications: "सूचनाएं",
    unread: "अपठित",
    "Mark read": "पढ़ा हुआ मार्क करें",
    "No notifications yet.": "अभी कोई सूचना नहीं।",
    "Close navigation": "नेविगेशन बंद करें",
    "Open navigation": "नेविगेशन खोलें",
    "Loading account": "खाता लोड हो रहा है",
    "Loading dashboard": "डैशबोर्ड लोड हो रहा है",
    "Sync complete.": "सिंक पूरा हुआ।",
    "Unable to sync right now.": "अभी सिंक नहीं हो पा रहा।",
    days: "दिन",
    cleared: "क्लियर",
    pending: "लंबित",
    failed: "असफल",
    "Monthly Budget": "मासिक बजट",
    "Rows needing attention": "ध्यान देने वाली पंक्तियां",
    "Simple category bar summary": "सरल श्रेणी बार सारांश",
    "Budget": "बजट",
    "Food merchants": "भोजन व्यापारी",
    "Travel merchants": "यात्रा व्यापारी",
    "Shopping merchants": "खरीदारी व्यापारी",
    "Search, add, and review every transaction before it hits reports.": "रिपोर्ट में जाने से पहले हर लेनदेन खोजें, जोड़ें और समीक्षा करें।",
    "Track limits, spot overspend early, and keep categories honest.": "सीमाएं ट्रैक करें, अतिरिक्त खर्च जल्दी पहचानें और श्रेणियां सही रखें।",
    "Triage low-confidence rows before posting them into the ledger.": "लेजर में डालने से पहले कम-विश्वास वाली पंक्तियों की समीक्षा करें।",
    "Track shared expenses, external participants, and settlements that need approval.": "साझा खर्च, बाहरी प्रतिभागी और अनुमोदन वाली निपटान ट्रैक करें।",
    "NovaCent reporting for category mix, cash flow, budget variance, merchants, parties, and currencies.": "श्रेणी मिश्रण, कैश फ्लो, बजट अंतर, व्यापारी, समूह और मुद्राओं के लिए NovaCent रिपोर्टिंग।",
    "Workspace preferences, sync posture, and localization readiness.": "कार्यक्षेत्र प्राथमिकताएं, सिंक स्थिति और भाषा तैयारी।",
    "Key account metrics": "मुख्य खाता मेट्रिक्स",
    "No expenses yet": "अभी कोई खर्च नहीं",
    "New spending will appear here once imported or added.": "आयात या जोड़ने के बाद नया खर्च यहां दिखाई देगा।",
    "Expense saved or queued for sync.": "खर्च सहेजा गया या सिंक कतार में है।",
    "Unable to save expense.": "खर्च सहेजा नहीं जा सका।",
    "Expense deleted.": "खर्च हटाया गया।",
    "Unable to delete expense. Settled party expenses and settlement rows are locked.": "खर्च हटाया नहीं जा सका। निपटे समूह खर्च और निपटान पंक्तियां लॉक हैं।",
    "Merchant, category, owner": "व्यापारी, श्रेणी, मालिक",
    "Budget name or category": "बजट नाम या श्रेणी",
    Tracks: "ट्रैक करता है",
    Limit: "सीमा",
    Period: "अवधि",
    "Alert at %": "% पर अलर्ट",
    "Save budget": "बजट सहेजें",
    "Update budget": "बजट अपडेट करें",
    "Budget saved.": "बजट सहेजा गया।",
    "Unable to save budget.": "बजट सहेजा नहीं जा सका।",
    "Budget deleted.": "बजट हटाया गया।",
    "Unable to delete budget.": "बजट हटाया नहीं जा सका।",
    "Jan 1 to Dec 31": "1 जनवरी से 31 दिसंबर",
    "1st of this month to month end": "इस महीने की 1 तारीख से महीने के अंत तक",
    "Uploading and parsing statement.": "स्टेटमेंट अपलोड और पढ़ा जा रहा है।",
    "Statement import failed.": "स्टेटमेंट आयात विफल हुआ।",
    "Statement rows are ready for review.": "स्टेटमेंट पंक्तियां समीक्षा के लिए तैयार हैं।",
    "Import row filter": "आयात पंक्ति फ़िल्टर",
    "Imported transaction review queue": "आयातित लेनदेन समीक्षा कतार",
    Source: "स्रोत",
    "Suggested category": "सुझाई गई श्रेणी",
    Confidence: "विश्वास",
    "possible duplicate": "संभावित डुप्लिकेट",
    "No rows match this filter": "इस फ़िल्टर से कोई पंक्ति मेल नहीं खाती",
    "Switch back to all rows to continue import review.": "आयात समीक्षा जारी रखने के लिए सभी पंक्तियों पर लौटें।",
    "Party deleted.": "समूह हटाया गया।",
    "Unable to delete party. Parties with settled expenses are locked.": "समूह हटाया नहीं जा सका। निपटे खर्च वाले समूह लॉक हैं।",
    "Party name": "समूह नाम",
    "Weekend friends": "वीकेंड दोस्त",
    "Save party": "समूह सहेजें",
    "Unable to save party.": "समूह सहेजा नहीं जा सका।",
    "Party saved.": "समूह सहेजा गया।",
    "No parties yet": "अभी कोई समूह नहीं",
    "Create a party to track shared expenses and settlements.": "साझा खर्च और निपटान ट्रैक करने के लिए समूह बनाएं।",
    "Loading party workspace": "समूह कार्यक्षेत्र लोड हो रहा है",
    "Unable to load party details.": "समूह विवरण लोड नहीं हो सके।",
    "Delete party": "समूह हटाएं",
    "Party participants": "समूह प्रतिभागी",
    "Search friend": "मित्र खोजें",
    "Name or email": "नाम या ईमेल",
    "as non-registered person": "गैर-पंजीकृत व्यक्ति के रूप में",
    "Participant list updated.": "प्रतिभागी सूची अपडेट हुई।",
    "Unable to update participant list.": "प्रतिभागी सूची अपडेट नहीं हो सकी।",
    "Save party expense": "समूह खर्च सहेजें",
    "Party expense and splits saved.": "समूह खर्च और विभाजन सहेजे गए।",
    "Unable to save party expense or splits.": "समूह खर्च या विभाजन सहेजे नहीं जा सके।",
    "Party expense deleted.": "समूह खर्च हटाया गया।",
    "Unable to delete party expense. Settled party expenses are locked.": "समूह खर्च हटाया नहीं जा सका। निपटे खर्च लॉक हैं।",
    "No party expenses yet": "अभी कोई समूह खर्च नहीं",
    "Add a party expense to start tracking splits.": "विभाजन ट्रैक करने के लिए समूह खर्च जोड़ें।",
    "Party settlement actions": "समूह निपटान कार्रवाइयां",
    "Party split ledger": "समूह विभाजन लेजर",
    "No open splits": "कोई खुला विभाजन नहीं",
    "Add a party expense to generate split settlement actions.": "निपटान कार्रवाइयां बनाने के लिए समूह खर्च जोड़ें।",
    "Approval requests": "अनुमोदन अनुरोध",
    "marked settled": "ने निपटा हुआ मार्क किया",
    "needs approval from": "को अनुमोदन चाहिए",
    "the payer": "भुगतानकर्ता",
    "before the split closes.": "विभाजन बंद होने से पहले।",
    "Waiting for": "प्रतीक्षा",
    "to review.": "समीक्षा के लिए।",
    "External placeholder settled directly.": "बाहरी प्लेसहोल्डर सीधे निपट गया।",
    "Settlement approval request created.": "निपटान अनुमोदन अनुरोध बनाया गया।",
    "Unable to mark this split as settled.": "इस विभाजन को निपटा हुआ मार्क नहीं कर सके।",
    "Settlement approved.": "निपटान स्वीकृत।",
    "Settlement rejected.": "निपटान अस्वीकृत।",
    "Unable to review settlement.": "निपटान की समीक्षा नहीं हो सकी।",
    "Across active report categories": "सक्रिय रिपोर्ट श्रेणियों में",
    "Income minus spend in visible months": "दिखने वाले महीनों में आय माइनस खर्च",
    "Highest active category utilization": "सबसे अधिक सक्रिय श्रेणी उपयोग",
    "Original spend currencies represented": "मूल खर्च मुद्राएं प्रदर्शित",
    "Currency exposure": "मुद्रा एक्सपोज़र",
    "Spend by original currency chart": "मूल मुद्रा के अनुसार खर्च चार्ट",
    "Preferences saved on this device.": "प्राथमिकताएं इस डिवाइस पर सहेजी गईं।",
    "Local preferences persist in browser storage.": "स्थानीय प्राथमिकताएं ब्राउज़र स्टोरेज में रहती हैं।",
    "Live APIs are used by default; set NEXT_PUBLIC_USE_MOCKS=true for demo data.": "डिफॉल्ट रूप से लाइव APIs उपयोग होती हैं; डेमो डेटा के लिए NEXT_PUBLIC_USE_MOCKS=true सेट करें।",
    "Legacy app-shell caches are cleared in live mode so old demo screens do not keep rendering after refresh.": "लाइव मोड में पुराने कैश साफ किए जाते हैं ताकि रिफ्रेश के बाद पुराने डेमो स्क्रीन न दिखें।",
    "Offline banner reacts to browser connectivity events.": "ऑफलाइन बैनर ब्राउज़र कनेक्टिविटी घटनाओं पर प्रतिक्रिया देता है।",
    "Use email/password or Google to open your account workspace.": "अपना खाता कार्यक्षेत्र खोलने के लिए ईमेल/पासवर्ड या Google का उपयोग करें।",
    "Invalid email or password.": "ईमेल या पासवर्ड गलत है।",
    "Create your account": "अपना खाता बनाएं",
    "A default INR account is created automatically after registration.": "रजिस्ट्रेशन के बाद डिफॉल्ट INR खाता अपने आप बनता है।",
    "Creating account": "खाता बन रहा है",
    "Unable to create account. The email may already be registered.": "खाता नहीं बन सका। ईमेल पहले से पंजीकृत हो सकता है।",
    "Already have an account?": "पहले से खाता है?",
    "We prepare a secure single-use reset token that expires after 30 minutes.": "हम एक सुरक्षित एक-बार उपयोग होने वाला रीसेट टोकन बनाते हैं जो 30 मिनट में समाप्त हो जाता है।",
    "Preparing link": "लिंक तैयार हो रहा है",
    "Send reset link": "रीसेट लिंक भेजें",
    "If the email exists, a reset link has been prepared.": "यदि ईमेल मौजूद है, तो रीसेट लिंक तैयार किया गया है।",
    "Open development reset link": "डेवलपमेंट रीसेट लिंक खोलें",
    "Back to sign in": "साइन इन पर वापस",
    "Choose a new password": "नया पासवर्ड चुनें",
    "Use at least 8 characters. The reset token can only be used once.": "कम से कम 8 अक्षर उपयोग करें। रीसेट टोकन केवल एक बार उपयोग हो सकता है।",
    "Updating password": "पासवर्ड अपडेट हो रहा है",
    "Reset token is invalid or expired.": "रीसेट टोकन अमान्य या समाप्त हो गया है।",
    "Category spend totals": "श्रेणी खर्च कुल",
    "Monthly cash flow": "मासिक कैश फ्लो",
    "Merchant trend lanes": "व्यापारी ट्रेंड लेन",
    "Party balances": "समूह बैलेंस",
    home: "होम",
    "bar chart": "बार चार्ट",
    "from expenses? This cannot be undone.": "खर्चों से हटाएं? यह वापस नहीं किया जा सकता।",
    "budget?": "बजट?",
    "and all unlocked party expenses?": "और सभी अनलॉक समूह खर्च?",
    "from this party and the overall expense ledger?": "इस समूह और पूरे खर्च लेजर से?",
    "match confidence": "मिलान विश्वास",
    "Party expense": "समूह खर्च",
    "Party expenses that feed the split ledger": "विभाजन लेजर में जाने वाले समूह खर्च",
    "Report summary metrics": "रिपोर्ट सारांश मेट्रिक्स",
    "Signing in": "साइन इन हो रहा है",
    "Loading NovaCent report charts": "NovaCent रिपोर्ट चार्ट लोड हो रहे हैं",
    "Monthly income, spend, and saved amount trend chart": "मासिक आय, खर्च और बचत ट्रेंड चार्ट",
    "Budget variance chart comparing actual spend with budget limit": "वास्तविक खर्च और बजट सीमा की तुलना वाला बजट अंतर चार्ट",
    "Merchant and category spending trends by month": "महीने के अनुसार व्यापारी और श्रेणी खर्च ट्रेंड",
    "Party outstanding and settled balance comparison": "समूह बकाया और निपटे बैलेंस की तुलना",
    "Monthly income, spend, and saved report": "मासिक आय, खर्च और बचत रिपोर्ट",
    "Budget variance by category": "श्रेणी के अनुसार बजट अंतर",
    "Party outstanding and settled balances": "समूह बकाया और निपटे बैलेंस",
    Party: "समूह",
    "Category spend chart with": "श्रेणी खर्च चार्ट जिसमें",
    data: "डेटा"
  },
  mr: {
    Workspace: "कार्यक्षेत्र",
    Loading: "लोड होत आहे",
    Saving: "जतन होत आहे",
    Syncing: "सिंक होत आहे",
    Delete: "हटवा",
    Cancel: "रद्द करा",
    Approve: "मंजूर करा",
    Reject: "नाकारा",
    Locked: "लॉक",
    Settled: "सेटल",
    "Admin only": "फक्त अॅडमिन",
    registered: "नोंदणीकृत",
    placeholder: "प्लेसहोल्डर",
    open: "उघडे",
    "approval pending": "मंजुरी प्रलंबित",
    "A calm command center for personal, party, and imported spending.": "वैयक्तिक, समूह आणि आयात केलेल्या खर्चांसाठी शांत नियंत्रण केंद्र.",
    "Across visible transactions this month": "या महिन्यातील दिसणाऱ्या व्यवहारांमध्ये",
    "Available across active envelopes": "सक्रिय बजेटमध्ये उपलब्ध",
    "Available in monthly budgets": "मासिक बजेटमध्ये उपलब्ध",
    "Available in yearly budgets": "वार्षिक बजेटमध्ये उपलब्ध",
    "At the current seven-day average": "सध्याच्या सात दिवसांच्या सरासरीनुसार",
    "Rows waiting for review": "पुनरावलोकनासाठी प्रतीक्षेत असलेल्या ओळी",
    "Budget health": "बजेट स्थिती",
    "Recent activity": "अलीकडील क्रिया",
    "Expense ledger": "खर्च वही",
    "Expense ledger with merchant, category, owner, amount, and status": "व्यापारी, श्रेणी, मालक, रक्कम आणि स्थितीसह खर्च वही",
    Action: "क्रिया",
    "Quick add": "जलद जोडा",
    Merchant: "व्यापारी",
    Category: "श्रेणी",
    Amount: "रक्कम",
    Currency: "चलन",
    Date: "तारीख",
    Owner: "मालक",
    Status: "स्थिती",
    Food: "अन्न",
    Groceries: "किराणा",
    Dining: "बाहेर खाणे",
    Shopping: "खरेदी",
    Travel: "प्रवास",
    "Bike Expense": "बाइक खर्च",
    Transport: "वाहतूक",
    Utilities: "युटिलिटी बिल",
    Rent: "भाडे",
    Education: "शिक्षण",
    Entertainment: "मनोरंजन",
    "Personal Care": "वैयक्तिक काळजी",
    Household: "घरगुती",
    Insurance: "विमा",
    Taxes: "कर",
    Gifts: "भेटवस्तू",
    Subscriptions: "सदस्यता",
    Health: "आरोग्य",
    Fuel: "इंधन",
    "Loan/EMI": "कर्ज/ईएमआय",
    Investments: "गुंतवणूक",
    Others: "इतर",
    "New budget": "नवीन बजेट",
    "Edit budget": "बजेट संपादित करा",
    "Active budgets": "सक्रिय बजेट",
    "Overall spend": "एकूण खर्च",
    "Single category": "एक श्रेणी",
    Monthly: "मासिक",
    Yearly: "वार्षिक",
    "this month": "या महिन्यात",
    "this year": "या वर्षी",
    month: "महिना",
    year: "वर्ष",
    of: "पैकी",
    spend: "खर्च",
    "Import review": "आयात पुनरावलोकन",
    "Upload statement": "स्टेटमेंट अपलोड करा",
    All: "सर्व",
    "Possible duplicates": "संभाव्य डुप्लिकेट",
    Parties: "समूह",
    "Back to parties": "समूहांकडे परत",
    "Create party": "समूह तयार करा",
    "Group balances": "समूह शिल्लक",
    "Open party": "समूह उघडा",
    workspace: "कार्यक्षेत्र",
    Participants: "सहभागी",
    Search: "शोधा",
    Add: "जोडा",
    "Add party expense": "समूह खर्च जोडा",
    "Paid by": "भरले",
    "Split mode": "वाटणी मोड",
    "Even split": "समान वाटणी",
    "Manual amounts": "मॅन्युअल रक्कम",
    owes: "देणे आहे",
    "Party expenses": "समूह खर्च",
    "Splits and settlements": "वाटणी आणि सेटलमेंट",
    Expense: "खर्च",
    "Owed by": "देय",
    Unknown: "अज्ञात",
    Participant: "सहभागी",
    "Mark settled": "सेटल मार्क करा",
    "Waiting approval": "मंजुरीची प्रतीक्षा",
    Reports: "अहवाल",
    "Export CSV": "CSV निर्यात",
    "Export PDF": "PDF निर्यात",
    "Tracked spend": "ट्रॅक केलेला खर्च",
    "Cash retained": "राखलेली रोख",
    "Largest budget usage": "सर्वाधिक बजेट वापर",
    Currencies: "चलने",
    "Spend by category": "श्रेणीनुसार खर्च",
    "Cash flow trend": "कॅश फ्लो ट्रेंड",
    "Budget variance": "बजेट फरक",
    "Merchant trends": "व्यापारी ट्रेंड",
    "Parties and currencies": "समूह आणि चलने",
    Preferences: "प्राधान्ये",
    "Default currency": "डिफॉल्ट चलन",
    "Offline readiness": "ऑफलाइन तयारी",
    "Sign in to": "साइन इन करा",
    Email: "ईमेल",
    Password: "पासवर्ड",
    "Sign in": "साइन इन",
    "Continue with Google": "Google सह पुढे जा",
    "Create an account": "खाते तयार करा",
    "Forgot password?": "पासवर्ड विसरलात?",
    Name: "नाव",
    "Create account": "खाते तयार करा",
    "Reset password": "पासवर्ड रीसेट करा",
    "New password": "नवीन पासवर्ड",
    "Update password": "पासवर्ड अपडेट करा",
    "Data table": "डेटा तक्ता",
    "Category mix": "श्रेणी मिश्रण",
    Spend: "खर्च",
    Month: "महिना",
    Income: "उत्पन्न",
    Saved: "बचत",
    Actual: "प्रत्यक्ष",
    Remaining: "उर्वरित",
    Used: "वापरले",
    Label: "लेबल",
    Outstanding: "बाकी",
    Notifications: "सूचना",
    unread: "न वाचलेले",
    "Mark read": "वाचलेले मार्क करा",
    "No notifications yet.": "अजून सूचना नाहीत.",
    "Close navigation": "नेव्हिगेशन बंद करा",
    "Open navigation": "नेव्हिगेशन उघडा",
    "Loading account": "खाते लोड होत आहे",
    "Loading dashboard": "डॅशबोर्ड लोड होत आहे",
    "Sync complete.": "सिंक पूर्ण झाले.",
    "Unable to sync right now.": "आत्ता सिंक होऊ शकत नाही.",
    days: "दिवस",
    cleared: "क्लिअर",
    pending: "प्रलंबित",
    failed: "अयशस्वी",
    "Monthly Budget": "मासिक बजेट",
    "Rows needing attention": "लक्ष देण्याच्या ओळी",
    "Simple category bar summary": "साधा श्रेणी बार सारांश",
    "Budget": "बजेट",
    "Food merchants": "अन्न व्यापारी",
    "Travel merchants": "प्रवास व्यापारी",
    "Shopping merchants": "खरेदी व्यापारी",
    home: "मुख्यपृष्ठ",
    "bar chart": "बार चार्ट",
    "from expenses? This cannot be undone.": "खर्चांमधून हटवायचे? हे परत करता येणार नाही.",
    "budget?": "बजेट?",
    "and all unlocked party expenses?": "आणि सर्व अनलॉक समूह खर्च?",
    "from this party and the overall expense ledger?": "या समूहातून आणि एकूण खर्च वहीतून?",
    "match confidence": "जुळणी विश्वास",
    "Alert at %": "% वर सूचना",
    "Already have an account?": "आधीच खाते आहे?",
    "Approval requests": "मंजुरी विनंत्या",
    "as non-registered person": "नोंदणीकृत नसलेली व्यक्ती म्हणून",
    "Back to sign in": "साइन इनकडे परत",
    "before the split closes.": "वाटणी बंद होण्यापूर्वी.",
    "Budget deleted.": "बजेट हटवले.",
    "Budget name or category": "बजेट नाव किंवा श्रेणी",
    "Budget saved.": "बजेट जतन झाले.",
    Confidence: "विश्वास",
    "Creating account": "खाते तयार होत आहे",
    "Delete party": "समूह हटवा",
    "Expense deleted.": "खर्च हटवला.",
    "Expense saved or queued for sync.": "खर्च जतन झाला किंवा सिंकसाठी रांगेत आहे.",
    "External placeholder settled directly.": "बाह्य प्लेसहोल्डर थेट सेटल झाला.",
    "If the email exists, a reset link has been prepared.": "ईमेल अस्तित्वात असल्यास रीसेट लिंक तयार केली आहे.",
    "Import row filter": "आयात ओळ फिल्टर",
    "Imported transaction review queue": "आयात व्यवहार पुनरावलोकन रांग",
    "Invalid email or password.": "ईमेल किंवा पासवर्ड चुकीचा आहे.",
    "Key account metrics": "मुख्य खाते मेट्रिक्स",
    "Legacy app-shell caches are cleared in live mode so old demo screens do not keep rendering after refresh.": "रिफ्रेशनंतर जुने डेमो स्क्रीन दिसू नयेत म्हणून लाइव्ह मोडमध्ये जुने अॅप-शेल कॅश साफ केले जातात.",
    Limit: "मर्यादा",
    "Live APIs are used by default; set NEXT_PUBLIC_USE_MOCKS=true for demo data.": "डिफॉल्टने लाइव्ह APIs वापरल्या जातात; डेमो डेटासाठी NEXT_PUBLIC_USE_MOCKS=true सेट करा.",
    "Local preferences persist in browser storage.": "स्थानिक प्राधान्ये ब्राउझर स्टोरेजमध्ये टिकून राहतात.",
    "marked settled": "ने सेटल मार्क केले",
    "Merchant, category, owner": "व्यापारी, श्रेणी, मालक",
    "Name or email": "नाव किंवा ईमेल",
    "needs approval from": "कडून मंजुरी आवश्यक आहे",
    "Offline banner reacts to browser connectivity events.": "ऑफलाइन बॅनर ब्राउझर कनेक्टिव्हिटी घटनांवर प्रतिक्रिया देतो.",
    "Open development reset link": "डेव्हलपमेंट रीसेट लिंक उघडा",
    "Participant list updated.": "सहभागी यादी अपडेट झाली.",
    "Party deleted.": "समूह हटवला.",
    "Party expense": "समूह खर्च",
    "Party expense and splits saved.": "समूह खर्च आणि वाटणी जतन झाली.",
    "Party expense deleted.": "समूह खर्च हटवला.",
    "Party expenses that feed the split ledger": "वाटणी वहीत जाणारे समूह खर्च",
    "Party name": "समूह नाव",
    "Party participants": "समूह सहभागी",
    "Party saved.": "समूह जतन झाला.",
    "Party settlement actions": "समूह सेटलमेंट क्रिया",
    "Party split ledger": "समूह वाटणी वही",
    Period: "कालावधी",
    "possible duplicate": "संभाव्य डुप्लिकेट",
    "Preferences saved on this device.": "प्राधान्ये या डिव्हाइसवर जतन झाली.",
    "Preparing link": "लिंक तयार होत आहे",
    "Report summary metrics": "अहवाल सारांश मेट्रिक्स",
    "Reset token is invalid or expired.": "रीसेट टोकन अवैध किंवा कालबाह्य आहे.",
    "Save budget": "बजेट जतन करा",
    "Save party": "समूह जतन करा",
    "Save party expense": "समूह खर्च जतन करा",
    "Search friend": "मित्र शोधा",
    "Send reset link": "रीसेट लिंक पाठवा",
    "Settlement approval request created.": "सेटलमेंट मंजुरी विनंती तयार झाली.",
    "Settlement approved.": "सेटलमेंट मंजूर झाले.",
    "Settlement rejected.": "सेटलमेंट नाकारले.",
    "Signing in": "साइन इन होत आहे",
    Source: "स्रोत",
    "Statement import failed.": "स्टेटमेंट आयात अयशस्वी झाली.",
    "Statement rows are ready for review.": "स्टेटमेंट ओळी पुनरावलोकनासाठी तयार आहेत.",
    "Suggested category": "सुचवलेली श्रेणी",
    "the payer": "भरणारा",
    "to review.": "पुनरावलोकनासाठी.",
    Tracks: "ट्रॅक करते",
    "Unable to create account. The email may already be registered.": "खाते तयार झाले नाही. ईमेल आधीच नोंदणीकृत असू शकतो.",
    "Unable to delete budget.": "बजेट हटवता आले नाही.",
    "Unable to delete expense. Settled party expenses and settlement rows are locked.": "खर्च हटवता आला नाही. सेटल झालेले समूह खर्च आणि सेटलमेंट ओळी लॉक आहेत.",
    "Unable to delete party expense. Settled party expenses are locked.": "समूह खर्च हटवता आला नाही. सेटल झालेले समूह खर्च लॉक आहेत.",
    "Unable to delete party. Parties with settled expenses are locked.": "समूह हटवता आला नाही. सेटल खर्च असलेले समूह लॉक आहेत.",
    "Unable to load party details.": "समूह तपशील लोड झाले नाहीत.",
    "Unable to mark this split as settled.": "ही वाटणी सेटल म्हणून मार्क करता आली नाही.",
    "Unable to review settlement.": "सेटलमेंटचे पुनरावलोकन करता आले नाही.",
    "Unable to save budget.": "बजेट जतन करता आले नाही.",
    "Unable to save expense.": "खर्च जतन करता आला नाही.",
    "Unable to save party expense or splits.": "समूह खर्च किंवा वाटणी जतन करता आली नाही.",
    "Unable to save party.": "समूह जतन करता आला नाही.",
    "Unable to update participant list.": "सहभागी यादी अपडेट करता आली नाही.",
    "Update budget": "बजेट अपडेट करा",
    "Updating password": "पासवर्ड अपडेट होत आहे",
    "Uploading and parsing statement.": "स्टेटमेंट अपलोड आणि वाचले जात आहे.",
    "Waiting for": "प्रतीक्षा",
    "Weekend friends": "वीकेंड मित्र",
    "Category spend totals": "श्रेणी खर्च एकूण",
    "Monthly cash flow": "मासिक कॅश फ्लो",
    "Merchant trend lanes": "व्यापारी ट्रेंड लेन",
    "Party balances": "समूह शिल्लक",
    "Loading NovaCent report charts": "NovaCent अहवाल चार्ट लोड होत आहेत",
    "Monthly income, spend, and saved amount trend chart": "मासिक उत्पन्न, खर्च आणि बचत ट्रेंड चार्ट",
    "Budget variance chart comparing actual spend with budget limit": "प्रत्यक्ष खर्च आणि बजेट मर्यादा तुलना करणारा बजेट फरक चार्ट",
    "Merchant and category spending trends by month": "महिन्यानुसार व्यापारी आणि श्रेणी खर्च ट्रेंड",
    "Party outstanding and settled balance comparison": "समूह बाकी आणि सेटल शिल्लक तुलना",
    "Monthly income, spend, and saved report": "मासिक उत्पन्न, खर्च आणि बचत अहवाल",
    "Budget variance by category": "श्रेणीनुसार बजेट फरक",
    "Party outstanding and settled balances": "समूह बाकी आणि सेटल शिल्लक",
    Party: "समूह",
    "Category spend chart with": "श्रेणी खर्च चार्ट ज्यात",
    data: "डेटा"
  }
};

export const supplementalPhraseDictionary: Record<Language, Record<string, string>> = {
  en: {
    Uncategorized: "Uncategorized",
    "Included expenses": "Included expenses",
    "No expenses included in this budget yet.": "No expenses included in this budget yet.",
    "Report timeframe": "Report timeframe",
    "Date range": "Date range",
    From: "From",
    To: "To",
    "Past 15 days": "Past 15 days",
    "1 month": "1 month",
    "3 months": "3 months",
    "1 year": "1 year",
    "3 years": "3 years",
    All: "All",
    "Approve all": "Approve all",
    "Approve except duplicates": "Approve except duplicates",
    record: "record",
    records: "records",
    "Unable to approve import rows.": "Unable to approve import rows.",
    Confirmation: "Confirmation",
    "Confirm delete": "Confirm delete",
    "This action cannot be undone.": "This action cannot be undone.",
    "from expenses?": "from expenses?",
    "Select all expenses": "Select all expenses",
    "Delete selected": "Delete selected",
    "Delete selected expenses?": "Delete selected expenses?",
    "Delete all": "Delete all",
    "Delete all import rows?": "Delete all import rows?",
    "Unable to delete import rows.": "Unable to delete import rows.",
    "Custom range": "Custom range",
    "Install app": "Install app",
    "Add this workspace to your device for quicker offline access.": "Add this workspace to your device for quicker offline access.",
    Install: "Install",
    "Not now": "Not now",
    "No report data for this timeframe": "No report data for this timeframe",
    "Try All or a wider date range to include more transactions.": "Try All or a wider date range to include more transactions.",
    "Open date picker": "Open date picker",
    Clear: "Clear",
    "Clear notifications": "Clear notifications",
    "How to use NovaCent": "How to use NovaCent",
    "New to NovaCent?": "New to NovaCent?",
    "Open the quick guide to understand accounts, imports, budgets, parties, reports, and offline sync before you start.": "Open the quick guide to understand accounts, imports, budgets, parties, reports, and offline sync before you start.",
    "View guide": "View guide",
    "Back to settings": "Back to settings",
    "Help and guidance": "Help and guidance",
    "Open guide": "Open guide",
    "Quick start": "Quick start",
    "Quick start checklist": "Quick start checklist",
    "NovaCent guide sections": "NovaCent guide sections",
    "NovaCent secure finance workspace": "NovaCent secure finance workspace",
    "Personal finance, neatly connected.": "Personal finance, neatly connected.",
    "Secure money workspace": "Secure money workspace",
    "Your money workspace, ready when you are.": "Your money workspace, ready when you are.",
    "Accounts, budgets, imports, parties, recurring expenses, and reports stay organized in one calm place.": "Accounts, budgets, imports, parties, recurring expenses, and reports stay organized in one calm place.",
    "NovaCent highlights": "NovaCent highlights",
    "Account scoped": "Account scoped",
    "Only your selected account data is shown.": "Only your selected account data is shown.",
    "Offline ready": "Offline ready",
    "Supported changes can wait safely for sync.": "Supported changes can wait safely for sync.",
    "Report friendly": "Report friendly",
    "Every approved input feeds your charts.": "Every approved input feeds your charts.",
    "Welcome back": "Welcome back",
    "Start your workspace": "Start your workspace",
    "Account recovery": "Account recovery",
    "Password reset": "Password reset",
    "Sign in to NovaCent": "Sign in to NovaCent",
    "Working on your request...": "Working on your request...",
    "Processing statement...": "Processing statement...",
    "Saving imported rows...": "Saving imported rows...",
    "Updating imported rows...": "Updating imported rows...",
    "Syncing changes...": "Syncing changes...",
    "Updating notifications...": "Updating notifications...",
    "Signing out...": "Signing out...",
    "Please wait while NovaCent finishes this request.": "Please wait while NovaCent finishes this request.",
    "Add at least one more participant before adding staged expenses.": "Add at least one more participant before adding staged expenses.",
    "Add another participant to create splits. With only one participant, NovaCent saves this as a party-only expense.": "Add another participant to create splits. With only one participant, NovaCent saves this as a party-only expense.",
    "Party expense saved without splits.": "Party expense saved without splits.",
    "Some selected expenses cannot be added to a split.": "Some selected expenses cannot be added to a split.",
    "This is used for new entries. Existing expenses keep the currency they were saved with.": "This is used for new entries. Existing expenses keep the currency they were saved with.",
    Investments: "Investments",
    Investment: "Investment",
    "Mutual Funds": "Mutual Funds",
    Stocks: "Stocks",
    "Postal Investments": "Postal Investments",
    "Insurance Investments": "Insurance Investments",
    "PPF/NPS": "PPF/NPS",
    "Fixed Deposits": "Fixed Deposits",
    "Recurring Deposits": "Recurring Deposits",
    Bonds: "Bonds",
    Gold: "Gold",
    Crypto: "Crypto",
    "Other Investments": "Other Investments",
    Type: "Type",
    Spend: "Spend",
    Income: "Income",
    Transfer: "Transfer",
    View: "View",
    "All transactions": "All transactions",
    "Actual spends only": "Actual spends only",
    Profile: "Profile",
    "Open profile": "Open profile",
    "View your account details, update your display name, and manage password access.": "View your account details, update your display name, and manage password access.",
    "Profile details": "Profile details",
    "Full name": "Full name",
    "Sign-in method": "Sign-in method",
    "Email and password": "Email and password",
    "Default account": "Default account",
    "Primary Account": "Primary Account",
    "Member since": "Member since",
    "Last updated": "Last updated",
    "User ID": "User ID",
    "Not available": "Not available",
    "Profile identifiers are read-only and managed by your account.": "Profile identifiers are read-only and managed by your account.",
    "Edit name": "Edit name",
    "This is the name friends see in parties, splits, and shared activity.": "This is the name friends see in parties, splits, and shared activity.",
    "Save name": "Save name",
    "Name updated.": "Name updated.",
    "Unable to update profile.": "Unable to update profile.",
    "Change password": "Change password",
    "Current password": "Current password",
    "Confirm new password": "Confirm new password",
    "Use at least 8 characters for the new password.": "Use at least 8 characters for the new password.",
    "Password changed.": "Password changed.",
    "Passwords do not match.": "Passwords do not match.",
    "Unable to change password.": "Unable to change password.",
    "Password change is only available for email/password accounts.": "Password change is only available for email/password accounts.",
    "Current password is incorrect.": "Current password is incorrect.",
    "Loading profile": "Loading profile",
    "Merchant or category": "Merchant or category",
    "Expense ledger with merchant, category, amount, and status": "Expense ledger with merchant, category, amount, and status",
    Timeframe: "Timeframe",
    "Investment ledger": "Investment ledger",
    "Filtered investment total": "Filtered investment total",
    "Filtered total": "Filtered total",
    "filtered transactions": "filtered transactions",
    "Loading investments": "Loading investments",
    "No investments yet": "No investments yet",
    "Investment transactions will appear here once added or imported.": "Investment transactions will appear here once added or imported.",
    "Expense classification updated.": "Expense classification updated.",
    "Unable to update expense classification.": "Unable to update expense classification.",
    "Investment classification updated.": "Investment classification updated.",
    "Unable to update investment classification.": "Unable to update investment classification.",
    "Review investment transactions separately from day-to-day spending.": "Review investment transactions separately from day-to-day spending.",
    "Invested this month": "Invested this month",
    "Invested this year": "Invested this year",
    "Investment activity in this timeframe": "Investment activity in this timeframe",
    "Investments by month": "Investments by month",
    "Investment trend chart": "Investment trend chart",
    manual: "manual",
    import: "import",
    recurring: "recurring",
    party: "party",
    trip: "trip",
    settlement: "settlement"
  },
  es: {
    "Monthy Budget": "Presupuesto mensual",
    "Statement upload controls": "Controles de carga de extracto",
    "Statement password": "Contrasena del extracto",
    "Optional for locked PDFs": "Opcional para PDFs bloqueados",
    "Ref no": "Referencia",
    Withdrawal: "Retiro",
    Deposit: "Deposito",
    Reimbursements: "Reembolsos",
    Uncategorized: "Sin categoria",
    "Included expenses": "Gastos incluidos",
    "No expenses included in this budget yet.": "Aun no hay gastos incluidos en este presupuesto.",
    "Report timeframe": "Periodo del informe",
    "Date range": "Rango de fechas",
    From: "Desde",
    To: "Hasta",
    "Past 15 days": "Ultimos 15 dias",
    "1 month": "1 mes",
    "3 months": "3 meses",
    "1 year": "1 ano",
    "3 years": "3 anos",
    All: "Todo",
    "Approve all": "Aprobar todo",
    "Approve except duplicates": "Aprobar sin duplicados",
    record: "registro",
    records: "registros",
    "Unable to approve import rows.": "No se pudieron aprobar las filas importadas.",
    Confirmation: "Confirmacion",
    "Confirm delete": "Confirmar eliminacion",
    "This action cannot be undone.": "Esta accion no se puede deshacer.",
    "from expenses?": "de gastos?",
    "Select all expenses": "Seleccionar todos los gastos",
    "Delete selected": "Eliminar seleccionados",
    "Delete selected expenses?": "Eliminar los gastos seleccionados?",
    "Delete all": "Eliminar todo",
    "Delete all import rows?": "Eliminar todas las filas importadas?",
    "Unable to delete import rows.": "No se pudieron eliminar las filas importadas.",
    "Custom range": "Rango personalizado",
    "Install app": "Instalar app",
    "Add this workspace to your device for quicker offline access.": "Agrega este espacio a tu dispositivo para acceso sin conexion mas rapido.",
    Install: "Instalar",
    "Not now": "Ahora no",
    "No report data for this timeframe": "No hay datos para este periodo",
    "Try All or a wider date range to include more transactions.": "Prueba Todo o un rango mas amplio para incluir mas transacciones.",
    "Open date picker": "Abrir selector de fecha",
    Clear: "Borrar",
    "Clear notifications": "Borrar notificaciones",
    "How to use NovaCent": "Como usar NovaCent",
    "New to NovaCent?": "Nuevo en NovaCent?",
    "Open the quick guide to understand accounts, imports, budgets, parties, reports, and offline sync before you start.": "Abre la guia rapida para entender cuentas, importaciones, presupuestos, grupos, informes y sincronizacion sin conexion antes de empezar.",
    "View guide": "Ver guia",
    "Back to settings": "Volver a ajustes",
    "Help and guidance": "Ayuda y guia",
    "Open guide": "Abrir guia",
    "Quick start": "Inicio rapido",
    "Quick start checklist": "Lista de inicio rapido",
    "NovaCent guide sections": "Secciones de guia de NovaCent",
    "NovaCent secure finance workspace": "Espacio financiero seguro de NovaCent",
    "Personal finance, neatly connected.": "Finanzas personales, bien conectadas.",
    "Secure money workspace": "Espacio financiero seguro",
    "Your money workspace, ready when you are.": "Tu espacio de dinero, listo cuando tu lo estes.",
    "Accounts, budgets, imports, parties, recurring expenses, and reports stay organized in one calm place.": "Cuentas, presupuestos, importaciones, grupos, gastos recurrentes e informes se mantienen ordenados en un solo lugar.",
    "NovaCent highlights": "Puntos clave de NovaCent",
    "Account scoped": "Por cuenta",
    "Only your selected account data is shown.": "Solo se muestran los datos de la cuenta seleccionada.",
    "Offline ready": "Listo sin conexion",
    "Supported changes can wait safely for sync.": "Los cambios compatibles pueden esperar de forma segura a sincronizarse.",
    "Report friendly": "Listo para informes",
    "Every approved input feeds your charts.": "Cada dato aprobado alimenta tus graficos.",
    "Welcome back": "Bienvenido de nuevo",
    "Start your workspace": "Empieza tu espacio",
    "Account recovery": "Recuperacion de cuenta",
    "Password reset": "Restablecimiento de contrasena",
    "Sign in to NovaCent": "Iniciar sesion en NovaCent",
    "Working on your request...": "Procesando tu solicitud...",
    "Processing statement...": "Procesando extracto...",
    "Saving imported rows...": "Guardando filas importadas...",
    "Updating imported rows...": "Actualizando filas importadas...",
    "Syncing changes...": "Sincronizando cambios...",
    "Updating notifications...": "Actualizando notificaciones...",
    "Signing out...": "Cerrando sesion...",
    "Please wait while NovaCent finishes this request.": "Espera mientras NovaCent termina esta solicitud.",
    manual: "manual",
    import: "importacion",
    recurring: "recurrente",
    party: "grupo",
    trip: "viaje",
    settlement: "liquidacion",
    "More details": "Mas detalles",
    "This statement is password protected. Enter the statement password and upload again.": "Este extracto esta protegido con contrasena. Ingresa la contrasena y vuelve a cargarlo.",
    "The statement password is incorrect. Please retry with the correct password.": "La contrasena del extracto es incorrecta. Intenta de nuevo con la contrasena correcta.",
    "Create recurring rules that post due expenses automatically in the background.": "Crea reglas recurrentes que registran gastos vencidos automaticamente en segundo plano.",
    "New recurring rule": "Nueva regla recurrente",
    "Edit recurring rule": "Editar regla recurrente",
    Description: "Descripcion",
    Frequency: "Frecuencia",
    Daily: "Diario",
    Weekly: "Semanal",
    Interval: "Intervalo",
    "Start date": "Fecha inicial",
    "End date": "Fecha final",
    Notes: "Notas",
    "Save recurring rule": "Guardar regla recurrente",
    "Update recurring rule": "Actualizar regla recurrente",
    "Recurring rule saved.": "Regla recurrente guardada.",
    "Unable to save recurring rule.": "No se pudo guardar la regla recurrente.",
    "Recurring rule updated.": "Regla recurrente actualizada.",
    "Unable to update recurring rule.": "No se pudo actualizar la regla recurrente.",
    "Recurring rules": "Reglas recurrentes",
    "Next run": "Proxima ejecucion",
    Edit: "Editar",
    Pause: "Pausar",
    Resume: "Reanudar",
    End: "Finalizar",
    active: "activa",
    paused: "pausada",
    ended: "finalizada",
    daily: "diario",
    weekly: "semanal",
    monthly: "mensual",
    yearly: "anual",
    "No recurring rules yet": "Aun no hay reglas recurrentes",
    "Create rules for expenses that should post automatically when due.": "Crea reglas para gastos que deben registrarse automaticamente al vencer.",
    Percentages: "Porcentajes",
    Shares: "Participaciones",
    shares: "participaciones",
    "Create party from selection": "Crear grupo desde seleccion",
    "Add selected expenses to split": "Agregar gastos seleccionados a division",
    "Existing party": "Grupo existente",
    "Choose party": "Elegir grupo",
    "Add to party split": "Agregar a division de grupo",
    "Create new party instead": "Crear nuevo grupo",
    "Selected expenses added to the party split.": "Gastos seleccionados agregados a la division del grupo.",
    "Unable to add selected expenses. Add at least two participants to the party first.": "No se pudieron agregar. Agrega al menos dos participantes al grupo.",
    "You have selected expenses waiting to be added to this party split.": "Tienes gastos seleccionados esperando agregarse a esta division.",
    "Add staged expenses": "Agregar gastos preparados",
    "Add at least one more participant before adding staged expenses.": "Agrega al menos un participante mas antes de agregar los gastos preparados.",
    "Add another participant to create splits. With only one participant, NovaCent saves this as a party-only expense.": "Agrega otro participante para crear divisiones. Con un solo participante, NovaCent guarda esto como un gasto solo del grupo.",
    "Party expense saved without splits.": "Gasto de grupo guardado sin divisiones.",
    "Some selected expenses cannot be added to a split.": "Algunos gastos seleccionados no se pueden agregar a una division.",
    "This is used for new entries. Existing expenses keep the currency they were saved with.": "Se usa para nuevas entradas. Los gastos existentes conservan la moneda con la que se guardaron.",
    Investment: "Inversion",
    "Mutual Funds": "Fondos mutuos",
    Stocks: "Acciones",
    "Postal Investments": "Inversiones postales",
    "Insurance Investments": "Inversiones de seguro",
    "PPF/NPS": "PPF/NPS",
    "Fixed Deposits": "Depositos fijos",
    "Recurring Deposits": "Depositos recurrentes",
    Bonds: "Bonos",
    Gold: "Oro",
    Crypto: "Cripto",
    "Other Investments": "Otras inversiones",
    Type: "Tipo",
    Spend: "Gasto",
    Income: "Ingreso",
    Transfer: "Transferencia",
    View: "Vista",
    "All transactions": "Todas las transacciones",
    "Actual spends only": "Solo gastos reales",
    Profile: "Perfil",
    "Open profile": "Abrir perfil",
    "View your account details, update your display name, and manage password access.": "Consulta tus datos, actualiza tu nombre visible y administra el acceso por contraseña.",
    "Profile details": "Detalles del perfil",
    "Full name": "Nombre completo",
    "Sign-in method": "Metodo de inicio de sesion",
    "Email and password": "Email y contraseña",
    "Default account": "Cuenta predeterminada",
    "Primary Account": "Cuenta principal",
    "Member since": "Miembro desde",
    "Last updated": "Ultima actualizacion",
    "User ID": "ID de usuario",
    "Not available": "No disponible",
    "Profile identifiers are read-only and managed by your account.": "Los identificadores del perfil son de solo lectura y se administran desde tu cuenta.",
    "Edit name": "Editar nombre",
    "This is the name friends see in parties, splits, and shared activity.": "Este es el nombre que tus amigos ven en grupos, divisiones y actividad compartida.",
    "Save name": "Guardar nombre",
    "Name updated.": "Nombre actualizado.",
    "Unable to update profile.": "No se pudo actualizar el perfil.",
    "Change password": "Cambiar contraseña",
    "Current password": "Contraseña actual",
    "Confirm new password": "Confirmar nueva contraseña",
    "Use at least 8 characters for the new password.": "Usa al menos 8 caracteres para la nueva contraseña.",
    "Password changed.": "Contraseña cambiada.",
    "Passwords do not match.": "Las contraseñas no coinciden.",
    "Unable to change password.": "No se pudo cambiar la contraseña.",
    "Password change is only available for email/password accounts.": "El cambio de contraseña solo esta disponible para cuentas con email y contraseña.",
    "Current password is incorrect.": "La contraseña actual es incorrecta.",
    "Loading profile": "Cargando perfil",
    "Merchant or category": "Comercio o categoria",
    "Expense ledger with merchant, category, amount, and status": "Libro de gastos con comercio, categoria, importe y estado",
    Timeframe: "Periodo",
    "Investment ledger": "Libro de inversiones",
    "Filtered investment total": "Total de inversiones filtradas",
    "Filtered total": "Total filtrado",
    "filtered transactions": "transacciones filtradas",
    "Loading investments": "Cargando inversiones",
    "No investments yet": "Aun no hay inversiones",
    "Investment transactions will appear here once added or imported.": "Las inversiones apareceran aqui cuando se agreguen o importen.",
    "Expense classification updated.": "Clasificacion del gasto actualizada.",
    "Unable to update expense classification.": "No se pudo actualizar la clasificacion del gasto.",
    "Investment classification updated.": "Clasificacion de inversion actualizada.",
    "Unable to update investment classification.": "No se pudo actualizar la clasificacion de inversion.",
    "Review investment transactions separately from day-to-day spending.": "Revisa las inversiones separadas del gasto diario.",
    "Invested this month": "Invertido este mes",
    "Invested this year": "Invertido este ano",
    "Investment activity in this timeframe": "Actividad de inversion en este periodo",
    "Investments by month": "Inversiones por mes",
    "Investment trend chart": "Grafico de tendencia de inversiones",
    "Customer support": "Soporte al cliente",
    "Send feature requests, issue reports, or praise to the NovaCent team.": "Envia solicitudes de funciones, reportes de problemas o elogios al equipo de NovaCent.",
    Name: "Nombre",
    "Request type": "Tipo de solicitud",
    "Add feature": "Agregar funcion",
    "Report issue": "Reportar problema",
    Praise: "Elogio",
    Comments: "Comentarios",
    "Send support request": "Enviar solicitud",
    "Support request saved.": "Solicitud de soporte guardada.",
    "Unable to save support request.": "No se pudo guardar la solicitud de soporte."
  },
  fr: {
    "Monthy Budget": "Budget mensuel",
    "Statement upload controls": "Commandes d'import de releve",
    "Statement password": "Mot de passe du releve",
    "Optional for locked PDFs": "Optionnel pour les PDF verrouilles",
    "Ref no": "Reference",
    Withdrawal: "Retrait",
    Deposit: "Depot",
    Reimbursements: "Remboursements",
    Uncategorized: "Non classe",
    "Included expenses": "Depenses incluses",
    "No expenses included in this budget yet.": "Aucune depense incluse dans ce budget pour l'instant.",
    "Report timeframe": "Periode du rapport",
    "Date range": "Plage de dates",
    From: "De",
    To: "A",
    "Past 15 days": "15 derniers jours",
    "1 month": "1 mois",
    "3 months": "3 mois",
    "1 year": "1 an",
    "3 years": "3 ans",
    All: "Tout",
    "Approve all": "Tout approuver",
    "Approve except duplicates": "Approuver sans doublons",
    record: "ligne",
    records: "lignes",
    "Unable to approve import rows.": "Impossible d'approuver les lignes importees.",
    Confirmation: "Confirmation",
    "Confirm delete": "Confirmer la suppression",
    "This action cannot be undone.": "Cette action est irreversible.",
    "from expenses?": "des depenses ?",
    "Select all expenses": "Selectionner toutes les depenses",
    "Delete selected": "Supprimer la selection",
    "Delete selected expenses?": "Supprimer les depenses selectionnees ?",
    "Delete all": "Tout supprimer",
    "Delete all import rows?": "Supprimer toutes les lignes importees ?",
    "Unable to delete import rows.": "Impossible de supprimer les lignes importees.",
    "Custom range": "Plage personnalisee",
    "Install app": "Installer l'app",
    "Add this workspace to your device for quicker offline access.": "Ajoutez cet espace a votre appareil pour un acces hors ligne plus rapide.",
    Install: "Installer",
    "Not now": "Pas maintenant",
    "No report data for this timeframe": "Aucune donnee pour cette periode",
    "Try All or a wider date range to include more transactions.": "Essayez Tout ou une plage plus large pour inclure plus de transactions.",
    "Open date picker": "Ouvrir le selecteur de date",
    Clear: "Effacer",
    "Clear notifications": "Effacer les notifications",
    "How to use NovaCent": "Comment utiliser NovaCent",
    "New to NovaCent?": "Nouveau sur NovaCent ?",
    "Open the quick guide to understand accounts, imports, budgets, parties, reports, and offline sync before you start.": "Ouvrez le guide rapide pour comprendre les comptes, imports, budgets, groupes, rapports et la synchronisation hors ligne avant de commencer.",
    "View guide": "Voir le guide",
    "Back to settings": "Retour aux parametres",
    "Help and guidance": "Aide et guide",
    "Open guide": "Ouvrir le guide",
    "Quick start": "Demarrage rapide",
    "Quick start checklist": "Liste de demarrage rapide",
    "NovaCent guide sections": "Sections du guide NovaCent",
    "NovaCent secure finance workspace": "Espace financier securise NovaCent",
    "Personal finance, neatly connected.": "Finances personnelles, bien reliees.",
    "Secure money workspace": "Espace financier securise",
    "Your money workspace, ready when you are.": "Votre espace financier, pret quand vous l'etes.",
    "Accounts, budgets, imports, parties, recurring expenses, and reports stay organized in one calm place.": "Comptes, budgets, imports, groupes, depenses recurrentes et rapports restent organises au meme endroit.",
    "NovaCent highlights": "Points forts de NovaCent",
    "Account scoped": "Par compte",
    "Only your selected account data is shown.": "Seules les donnees du compte selectionne sont affichees.",
    "Offline ready": "Pret hors ligne",
    "Supported changes can wait safely for sync.": "Les modifications prises en charge peuvent attendre la synchronisation en securite.",
    "Report friendly": "Pret pour les rapports",
    "Every approved input feeds your charts.": "Chaque entree approuvee alimente vos graphiques.",
    "Welcome back": "Bon retour",
    "Start your workspace": "Demarrer votre espace",
    "Account recovery": "Recuperation du compte",
    "Password reset": "Reinitialisation du mot de passe",
    "Sign in to NovaCent": "Se connecter a NovaCent",
    "Working on your request...": "Traitement de votre demande...",
    "Processing statement...": "Traitement du releve...",
    "Saving imported rows...": "Enregistrement des lignes importees...",
    "Updating imported rows...": "Mise a jour des lignes importees...",
    "Syncing changes...": "Synchronisation des changements...",
    "Updating notifications...": "Mise a jour des notifications...",
    "Signing out...": "Deconnexion...",
    "Please wait while NovaCent finishes this request.": "Veuillez patienter pendant que NovaCent termine cette demande.",
    manual: "manuel",
    import: "import",
    recurring: "recurrent",
    party: "groupe",
    trip: "voyage",
    settlement: "reglement",
    "More details": "Plus de details",
    "This statement is password protected. Enter the statement password and upload again.": "Ce releve est protege par mot de passe. Saisissez-le et importez a nouveau.",
    "The statement password is incorrect. Please retry with the correct password.": "Le mot de passe du releve est incorrect. Reessayez avec le bon mot de passe.",
    "Create recurring rules that post due expenses automatically in the background.": "Creez des regles recurrentes qui publient automatiquement les depenses dues en arriere-plan.",
    "New recurring rule": "Nouvelle regle recurrente",
    "Edit recurring rule": "Modifier la regle recurrente",
    Description: "Description",
    Frequency: "Frequence",
    Daily: "Quotidien",
    Weekly: "Hebdomadaire",
    Interval: "Intervalle",
    "Start date": "Date de debut",
    "End date": "Date de fin",
    Notes: "Notes",
    "Save recurring rule": "Enregistrer la regle recurrente",
    "Update recurring rule": "Mettre a jour la regle recurrente",
    "Recurring rule saved.": "Regle recurrente enregistree.",
    "Unable to save recurring rule.": "Impossible d'enregistrer la regle recurrente.",
    "Recurring rule updated.": "Regle recurrente mise a jour.",
    "Unable to update recurring rule.": "Impossible de mettre a jour la regle recurrente.",
    "Recurring rules": "Regles recurrentes",
    "Next run": "Prochaine execution",
    Edit: "Modifier",
    Pause: "Suspendre",
    Resume: "Reprendre",
    End: "Terminer",
    active: "active",
    paused: "suspendue",
    ended: "terminee",
    daily: "quotidien",
    weekly: "hebdomadaire",
    monthly: "mensuel",
    yearly: "annuel",
    "No recurring rules yet": "Aucune regle recurrente pour l'instant",
    "Create rules for expenses that should post automatically when due.": "Creez des regles pour les depenses qui doivent etre publiees automatiquement a echeance.",
    Percentages: "Pourcentages",
    Shares: "Parts",
    shares: "parts",
    "Create party from selection": "Creer un groupe depuis la selection",
    "Add selected expenses to split": "Ajouter les depenses selectionnees au partage",
    "Existing party": "Groupe existant",
    "Choose party": "Choisir un groupe",
    "Add to party split": "Ajouter au partage",
    "Create new party instead": "Creer un nouveau groupe",
    "Selected expenses added to the party split.": "Depenses selectionnees ajoutees au partage.",
    "Unable to add selected expenses. Add at least two participants to the party first.": "Impossible d'ajouter ces depenses. Ajoutez d'abord deux participants.",
    "You have selected expenses waiting to be added to this party split.": "Des depenses selectionnees attendent d'etre ajoutees a ce partage.",
    "Add staged expenses": "Ajouter les depenses preparees",
    "Add at least one more participant before adding staged expenses.": "Ajoutez au moins un participant de plus avant d'ajouter les depenses preparees.",
    "Add another participant to create splits. With only one participant, NovaCent saves this as a party-only expense.": "Ajoutez un autre participant pour creer des partages. Avec un seul participant, NovaCent enregistre cela comme une depense de groupe seule.",
    "Party expense saved without splits.": "Depense de groupe enregistree sans partages.",
    "Some selected expenses cannot be added to a split.": "Certaines depenses selectionnees ne peuvent pas etre ajoutees au partage.",
    "This is used for new entries. Existing expenses keep the currency they were saved with.": "Utilisee pour les nouvelles entrees. Les depenses existantes gardent la devise enregistree.",
    Investment: "Investissement",
    "Mutual Funds": "Fonds communs",
    Stocks: "Actions",
    "Postal Investments": "Investissements postaux",
    "Insurance Investments": "Investissements assurance",
    "PPF/NPS": "PPF/NPS",
    "Fixed Deposits": "Depots fixes",
    "Recurring Deposits": "Depots recurrents",
    Bonds: "Obligations",
    Gold: "Or",
    Crypto: "Crypto",
    "Other Investments": "Autres investissements",
    Type: "Type",
    Spend: "Depense",
    Income: "Revenu",
    Transfer: "Transfert",
    View: "Vue",
    "All transactions": "Toutes les transactions",
    "Actual spends only": "Depenses reelles uniquement",
    Profile: "Profil",
    "Open profile": "Ouvrir le profil",
    "View your account details, update your display name, and manage password access.": "Consultez vos details, modifiez votre nom affiche et gerez l'acces par mot de passe.",
    "Profile details": "Details du profil",
    "Full name": "Nom complet",
    "Sign-in method": "Mode de connexion",
    "Email and password": "Email et mot de passe",
    "Default account": "Compte par defaut",
    "Primary Account": "Compte principal",
    "Member since": "Membre depuis",
    "Last updated": "Derniere mise a jour",
    "User ID": "ID utilisateur",
    "Not available": "Non disponible",
    "Profile identifiers are read-only and managed by your account.": "Les identifiants du profil sont en lecture seule et geres par votre compte.",
    "Edit name": "Modifier le nom",
    "This is the name friends see in parties, splits, and shared activity.": "C'est le nom visible par vos amis dans les groupes, partages et activites partagees.",
    "Save name": "Enregistrer le nom",
    "Name updated.": "Nom mis a jour.",
    "Unable to update profile.": "Impossible de mettre a jour le profil.",
    "Change password": "Changer le mot de passe",
    "Current password": "Mot de passe actuel",
    "Confirm new password": "Confirmer le nouveau mot de passe",
    "Use at least 8 characters for the new password.": "Utilisez au moins 8 caracteres pour le nouveau mot de passe.",
    "Password changed.": "Mot de passe modifie.",
    "Passwords do not match.": "Les mots de passe ne correspondent pas.",
    "Unable to change password.": "Impossible de changer le mot de passe.",
    "Password change is only available for email/password accounts.": "Le changement de mot de passe est reserve aux comptes email/mot de passe.",
    "Current password is incorrect.": "Le mot de passe actuel est incorrect.",
    "Loading profile": "Chargement du profil",
    "Merchant or category": "Marchand ou categorie",
    "Expense ledger with merchant, category, amount, and status": "Registre avec marchand, categorie, montant et statut",
    Timeframe: "Periode",
    "Investment ledger": "Registre des investissements",
    "Filtered investment total": "Total des investissements filtres",
    "Filtered total": "Total filtre",
    "filtered transactions": "transactions filtrees",
    "Loading investments": "Chargement des investissements",
    "No investments yet": "Aucun investissement pour l'instant",
    "Investment transactions will appear here once added or imported.": "Les investissements apparaitront ici apres ajout ou import.",
    "Expense classification updated.": "Classification de depense mise a jour.",
    "Unable to update expense classification.": "Impossible de mettre a jour la classification.",
    "Investment classification updated.": "Classification d'investissement mise a jour.",
    "Unable to update investment classification.": "Impossible de mettre a jour la classification d'investissement.",
    "Review investment transactions separately from day-to-day spending.": "Consultez les investissements separement des depenses quotidiennes.",
    "Invested this month": "Investi ce mois-ci",
    "Invested this year": "Investi cette annee",
    "Investment activity in this timeframe": "Activite d'investissement sur cette periode",
    "Investments by month": "Investissements par mois",
    "Investment trend chart": "Graphique de tendance des investissements",
    "Customer support": "Support client",
    "Send feature requests, issue reports, or praise to the NovaCent team.": "Envoyez des demandes de fonctionnalites, des signalements ou des compliments a l'equipe NovaCent.",
    Name: "Nom",
    "Request type": "Type de demande",
    "Add feature": "Ajouter une fonctionnalite",
    "Report issue": "Signaler un probleme",
    Praise: "Felicitations",
    Comments: "Commentaires",
    "Send support request": "Envoyer la demande",
    "Support request saved.": "Demande de support enregistree.",
    "Unable to save support request.": "Impossible d'enregistrer la demande de support."
  },
  hi: {
    "Monthy Budget": "मासिक बजट",
    "Statement upload controls": "स्टेटमेंट अपलोड नियंत्रण",
    "Statement password": "स्टेटमेंट पासवर्ड",
    "Optional for locked PDFs": "लॉक PDF के लिए वैकल्पिक",
    "Ref no": "रेफ नंबर",
    Withdrawal: "निकासी",
    Deposit: "जमा",
    Reimbursements: "प्रतिपूर्ति",
    Uncategorized: "अवर्गीकृत",
    "Included expenses": "शामिल खर्च",
    "No expenses included in this budget yet.": "इस बजट में अभी कोई खर्च शामिल नहीं है.",
    "Report timeframe": "रिपोर्ट समयावधि",
    "Date range": "तारीख सीमा",
    From: "से",
    To: "तक",
    "Past 15 days": "पिछले 15 दिन",
    "1 month": "1 महीना",
    "3 months": "3 महीने",
    "1 year": "1 साल",
    "3 years": "3 साल",
    All: "सभी",
    "Approve all": "सभी मंजूर करें",
    "Approve except duplicates": "डुप्लिकेट छोड़कर मंजूर करें",
    "Unable to approve import rows.": "आयात पंक्तियां मंजूर नहीं हो सकीं.",
    Confirmation: "पुष्टि",
    "Confirm delete": "हटाने की पुष्टि करें",
    "This action cannot be undone.": "यह कार्रवाई वापस नहीं की जा सकती.",
    "from expenses?": "खर्चों से?",
    "Select all expenses": "सभी खर्च चुनें",
    "Delete selected": "चयनित हटाएं",
    "Delete selected expenses?": "चयनित खर्च हटाएं?",
    "Delete all": "सभी हटाएं",
    "Delete all import rows?": "सभी आयात पंक्तियां हटाएं?",
    "Unable to delete import rows.": "आयात पंक्तियां हटाई नहीं जा सकीं.",
    "Custom range": "कस्टम सीमा",
    "Install app": "ऐप इंस्टॉल करें",
    "Add this workspace to your device for quicker offline access.": "तेज ऑफलाइन एक्सेस के लिए इस वर्कस्पेस को अपने डिवाइस में जोड़ें.",
    Install: "इंस्टॉल",
    "Not now": "अभी नहीं",
    "No report data for this timeframe": "इस समयावधि के लिए रिपोर्ट डेटा नहीं है",
    "Try All or a wider date range to include more transactions.": "अधिक लेनदेन शामिल करने के लिए सभी या बड़ी तारीख सीमा चुनें.",
    "Open date picker": "तारीख पिकर खोलें",
    Clear: "साफ करें",
    "Clear notifications": "सूचनाएं साफ करें",
    "How to use NovaCent": "NovaCent का उपयोग कैसे करें",
    "Back to settings": "सेटिंग्स पर वापस जाएं",
    "Help and guidance": "सहायता और मार्गदर्शन",
    "Open guide": "गाइड खोलें",
    "Quick start": "त्वरित शुरुआत",
    "Quick start checklist": "त्वरित शुरुआत सूची",
    "NovaCent guide sections": "NovaCent गाइड सेक्शन",
    "NovaCent secure finance workspace": "NovaCent सुरक्षित वित्त कार्यक्षेत्र",
    "Personal finance, neatly connected.": "निजी वित्त, साफ़ तरीके से जुड़ा हुआ.",
    "Secure money workspace": "सुरक्षित वित्त कार्यक्षेत्र",
    "Your money workspace, ready when you are.": "आपका वित्त कार्यक्षेत्र, जब आप तैयार हों.",
    "Accounts, budgets, imports, parties, recurring expenses, and reports stay organized in one calm place.": "खाते, बजट, आयात, समूह, आवर्ती खर्च और रिपोर्ट एक शांत जगह पर व्यवस्थित रहते हैं.",
    "NovaCent highlights": "NovaCent विशेषताएं",
    "Account scoped": "खाता-आधारित",
    "Only your selected account data is shown.": "केवल चुने हुए खाते का डेटा दिखाया जाता है.",
    "Offline ready": "ऑफलाइन तैयार",
    "Supported changes can wait safely for sync.": "समर्थित बदलाव सुरक्षित रूप से सिंक का इंतजार कर सकते हैं.",
    "Report friendly": "रिपोर्ट के लिए तैयार",
    "Every approved input feeds your charts.": "हर स्वीकृत इनपुट आपके चार्ट में शामिल होता है.",
    "Welcome back": "वापसी पर स्वागत है",
    "Start your workspace": "अपना कार्यक्षेत्र शुरू करें",
    "Account recovery": "खाता पुनर्प्राप्ति",
    "Password reset": "पासवर्ड रीसेट",
    "Sign in to NovaCent": "NovaCent में साइन इन करें",
    "Working on your request...": "आपका अनुरोध पूरा हो रहा है...",
    "Processing statement...": "स्टेटमेंट प्रोसेस हो रहा है...",
    "Saving imported rows...": "आयात पंक्तियां सहेजी जा रही हैं...",
    "Updating imported rows...": "आयात पंक्तियां अपडेट हो रही हैं...",
    "Syncing changes...": "बदलाव सिंक हो रहे हैं...",
    "Updating notifications...": "सूचनाएं अपडेट हो रही हैं...",
    "Signing out...": "साइन आउट हो रहा है...",
    "Please wait while NovaCent finishes this request.": "कृपया प्रतीक्षा करें, NovaCent यह अनुरोध पूरा कर रहा है.",
    manual: "मैनुअल",
    import: "आयात",
    recurring: "आवर्ती",
    party: "समूह",
    trip: "यात्रा",
    settlement: "निपटान",
    "More details": "अधिक विवरण",
    "This statement is password protected. Enter the statement password and upload again.": "यह स्टेटमेंट पासवर्ड से सुरक्षित है। पासवर्ड दर्ज करके फिर से अपलोड करें।",
    "The statement password is incorrect. Please retry with the correct password.": "स्टेटमेंट पासवर्ड गलत है। सही पासवर्ड के साथ फिर कोशिश करें।",
    "Create recurring rules that post due expenses automatically in the background.": "ऐसे आवर्ती नियम बनाएं जो देय खर्चों को बैकग्राउंड में अपने आप पोस्ट करें।",
    "New recurring rule": "नई आवर्ती नियम",
    "Edit recurring rule": "आवर्ती नियम संपादित करें",
    Description: "विवरण",
    Frequency: "आवृत्ति",
    Daily: "दैनिक",
    Weekly: "साप्ताहिक",
    Interval: "अंतराल",
    "Start date": "शुरू तारीख",
    "End date": "अंत तारीख",
    Notes: "नोट्स",
    "Save recurring rule": "आवर्ती नियम सहेजें",
    "Update recurring rule": "आवर्ती नियम अपडेट करें",
    "Recurring rule saved.": "आवर्ती नियम सहेजा गया।",
    "Unable to save recurring rule.": "आवर्ती नियम सहेजा नहीं जा सका।",
    "Recurring rule updated.": "आवर्ती नियम अपडेट हो गया।",
    "Unable to update recurring rule.": "आवर्ती नियम अपडेट नहीं हो सका।",
    "Recurring rules": "आवर्ती नियम",
    "Next run": "अगली बार",
    Edit: "संपादित करें",
    Pause: "रोकें",
    Resume: "फिर शुरू करें",
    End: "समाप्त",
    active: "सक्रिय",
    paused: "रुका हुआ",
    ended: "समाप्त",
    daily: "दैनिक",
    weekly: "साप्ताहिक",
    monthly: "मासिक",
    yearly: "वार्षिक",
    "No recurring rules yet": "अभी कोई आवर्ती नियम नहीं",
    "Create rules for expenses that should post automatically when due.": "उन खर्चों के लिए नियम बनाएं जिन्हें देय होने पर अपने आप पोस्ट होना चाहिए।",
    Percentages: "प्रतिशत",
    Shares: "शेयर",
    shares: "शेयर",
    "Create party from selection": "चयन से समूह बनाएं",
    "Add selected expenses to split": "चयनित खर्च विभाजन में जोड़ें",
    "Existing party": "मौजूदा समूह",
    "Choose party": "समूह चुनें",
    "Add to party split": "समूह विभाजन में जोड़ें",
    "Create new party instead": "नया समूह बनाएं",
    "Add staged expenses": "तैयार खर्च जोड़ें",
    "Add at least one more participant before adding staged expenses.": "तैयार खर्च जोड़ने से पहले कम से कम एक और प्रतिभागी जोड़ें.",
    "Some selected expenses cannot be added to a split.": "कुछ चुने हुए खर्च विभाजन में नहीं जोड़े जा सकते.",
    "This is used for new entries. Existing expenses keep the currency they were saved with.": "यह नई एंट्री के लिए इस्तेमाल होता है. मौजूदा खर्च उसी मुद्रा में रहेंगे जिसमें वे सेव हुए थे.",
    Investment: "निवेश",
    "Mutual Funds": "म्यूचुअल फंड",
    Stocks: "स्टॉक",
    "Postal Investments": "डाक निवेश",
    "Insurance Investments": "बीमा निवेश",
    "PPF/NPS": "PPF/NPS",
    "Fixed Deposits": "फिक्स्ड डिपॉजिट",
    "Recurring Deposits": "रिकरिंग डिपॉजिट",
    Bonds: "बॉन्ड",
    Gold: "सोना",
    Crypto: "क्रिप्टो",
    "Other Investments": "अन्य निवेश",
    Type: "प्रकार",
    Spend: "खर्च",
    Income: "आय",
    Transfer: "ट्रांसफर",
    View: "देखें",
    "All transactions": "सभी लेनदेन",
    "Actual spends only": "केवल वास्तविक खर्च",
    Profile: "प्रोफ़ाइल",
    "Open profile": "प्रोफ़ाइल खोलें",
    "View your account details, update your display name, and manage password access.": "अपना खाता विवरण देखें, दिखने वाला नाम अपडेट करें और पासवर्ड एक्सेस संभालें.",
    "Profile details": "प्रोफ़ाइल विवरण",
    "Full name": "पूरा नाम",
    "Sign-in method": "साइन-इन तरीका",
    "Email and password": "ईमेल और पासवर्ड",
    "Default account": "डिफ़ॉल्ट खाता",
    "Primary Account": "प्राथमिक खाता",
    "Member since": "सदस्य बने",
    "Last updated": "अंतिम अपडेट",
    "User ID": "यूज़र ID",
    "Not available": "उपलब्ध नहीं",
    "Profile identifiers are read-only and managed by your account.": "प्रोफ़ाइल पहचान विवरण केवल पढ़ने के लिए हैं और आपके खाते से प्रबंधित होते हैं.",
    "Edit name": "नाम संपादित करें",
    "This is the name friends see in parties, splits, and shared activity.": "यह वही नाम है जो दोस्त समूह, स्प्लिट और साझा गतिविधि में देखते हैं.",
    "Save name": "नाम सेव करें",
    "Name updated.": "नाम अपडेट हुआ.",
    "Unable to update profile.": "प्रोफ़ाइल अपडेट नहीं हो सकी.",
    "Change password": "पासवर्ड बदलें",
    "Current password": "मौजूदा पासवर्ड",
    "Confirm new password": "नया पासवर्ड पुष्टि करें",
    "Use at least 8 characters for the new password.": "नए पासवर्ड के लिए कम से कम 8 अक्षर इस्तेमाल करें.",
    "Password changed.": "पासवर्ड बदल गया.",
    "Passwords do not match.": "पासवर्ड मेल नहीं खाते.",
    "Unable to change password.": "पासवर्ड बदला नहीं जा सका.",
    "Password change is only available for email/password accounts.": "पासवर्ड बदलना केवल ईमेल/पासवर्ड खातों के लिए उपलब्ध है.",
    "Current password is incorrect.": "मौजूदा पासवर्ड गलत है.",
    "Loading profile": "प्रोफ़ाइल लोड हो रही है",
    "Merchant or category": "व्यापारी या श्रेणी",
    "Expense ledger with merchant, category, amount, and status": "व्यापारी, श्रेणी, राशि और स्थिति वाला खर्च लेजर",
    Timeframe: "अवधि",
    "Investment ledger": "निवेश लेजर",
    "Filtered investment total": "फ़िल्टर किए गए निवेश का कुल",
    "Filtered total": "फ़िल्टर किया गया कुल",
    "filtered transactions": "फ़िल्टर किए गए लेनदेन",
    "Loading investments": "निवेश लोड हो रहे हैं",
    "No investments yet": "अभी कोई निवेश नहीं",
    "Investment transactions will appear here once added or imported.": "जोड़े या आयात किए गए निवेश यहां दिखेंगे.",
    "Expense classification updated.": "खर्च वर्गीकरण अपडेट हुआ.",
    "Unable to update expense classification.": "खर्च वर्गीकरण अपडेट नहीं हो सका.",
    "Investment classification updated.": "निवेश वर्गीकरण अपडेट हुआ.",
    "Unable to update investment classification.": "निवेश वर्गीकरण अपडेट नहीं हो सका.",
    "Review investment transactions separately from day-to-day spending.": "निवेश लेनदेन को रोज़ाना खर्च से अलग देखें.",
    "Invested this month": "इस महीने निवेश",
    "Invested this year": "इस साल निवेश",
    "Investment activity in this timeframe": "इस अवधि की निवेश गतिविधि",
    "Investments by month": "महीने के अनुसार निवेश",
    "Investment trend chart": "निवेश ट्रेंड चार्ट",
    "Customer support": "ग्राहक सहायता",
    "Send feature requests, issue reports, or praise to the NovaCent team.": "NovaCent टीम को फीचर अनुरोध, समस्या रिपोर्ट या प्रशंसा भेजें।",
    Name: "नाम",
    "Request type": "अनुरोध प्रकार",
    "Add feature": "फीचर जोड़ें",
    "Report issue": "समस्या बताएं",
    Praise: "प्रशंसा",
    Comments: "टिप्पणी",
    "Send support request": "सहायता अनुरोध भेजें"
  },
  mr: {
    "Monthy Budget": "मासिक बजेट",
    "Statement upload controls": "स्टेटमेंट अपलोड नियंत्रण",
    "Statement password": "स्टेटमेंट पासवर्ड",
    "Optional for locked PDFs": "लॉक PDF साठी ऐच्छिक",
    "Ref no": "रेफ नंबर",
    Withdrawal: "पैसे काढणे",
    Deposit: "जमा",
    Reimbursements: "परतावे",
    Uncategorized: "अवर्गीकृत",
    "Included expenses": "समाविष्ट खर्च",
    "No expenses included in this budget yet.": "या बजेटमध्ये अजून कोणताही खर्च समाविष्ट नाही.",
    "Report timeframe": "अहवाल कालावधी",
    "Date range": "तारीख श्रेणी",
    From: "पासून",
    To: "पर्यंत",
    "Past 15 days": "मागील 15 दिवस",
    "1 month": "1 महिना",
    "3 months": "3 महिने",
    "1 year": "1 वर्ष",
    "3 years": "3 वर्षे",
    All: "सर्व",
    "Approve all": "सर्व मंजूर करा",
    "Approve except duplicates": "डुप्लिकेट वगळून मंजूर करा",
    "Unable to approve import rows.": "आयात ओळी मंजूर करता आल्या नाहीत.",
    Confirmation: "पुष्टी",
    "Confirm delete": "हटवण्याची पुष्टी करा",
    "This action cannot be undone.": "ही कृती परत करता येणार नाही.",
    "from expenses?": "खर्चांमधून?",
    "Select all expenses": "सर्व खर्च निवडा",
    "Delete selected": "निवडलेले हटवा",
    "Delete selected expenses?": "निवडलेले खर्च हटवायचे?",
    "Delete all": "सर्व हटवा",
    "Delete all import rows?": "सर्व आयात ओळी हटवायच्या?",
    "Unable to delete import rows.": "आयात ओळी हटवता आल्या नाहीत.",
    "Custom range": "सानुकूल श्रेणी",
    "Install app": "अॅप इंस्टॉल करा",
    "Add this workspace to your device for quicker offline access.": "जलद ऑफलाइन प्रवेशासाठी हे वर्कस्पेस तुमच्या डिव्हाइसवर जोडा.",
    Install: "इंस्टॉल",
    "Not now": "आत्ता नाही",
    "No report data for this timeframe": "या कालावधीसाठी अहवाल डेटा नाही",
    "Try All or a wider date range to include more transactions.": "अधिक व्यवहार समाविष्ट करण्यासाठी सर्व किंवा मोठी तारीख श्रेणी वापरा.",
    "Open date picker": "तारीख निवडणारा उघडा",
    Clear: "साफ करा",
    "Clear notifications": "सूचना साफ करा",
    "How to use NovaCent": "NovaCent कसे वापरावे",
    "Back to settings": "सेटिंग्जवर परत जा",
    "Help and guidance": "मदत आणि मार्गदर्शन",
    "Open guide": "गाइड उघडा",
    "Quick start": "जलद सुरुवात",
    "Quick start checklist": "जलद सुरुवात यादी",
    "NovaCent guide sections": "NovaCent गाइड विभाग",
    "NovaCent secure finance workspace": "NovaCent सुरक्षित वित्त कार्यक्षेत्र",
    "Personal finance, neatly connected.": "वैयक्तिक वित्त, नीट जोडलेले.",
    "Secure money workspace": "सुरक्षित वित्त कार्यक्षेत्र",
    "Your money workspace, ready when you are.": "तुमचे वित्त कार्यक्षेत्र, तुम्ही तयार असताना तयार.",
    "Accounts, budgets, imports, parties, recurring expenses, and reports stay organized in one calm place.": "खाती, बजेट, आयात, समूह, आवर्ती खर्च आणि अहवाल एका शांत ठिकाणी नीट राहतात.",
    "NovaCent highlights": "NovaCent वैशिष्ट्ये",
    "Account scoped": "खात्यानुसार",
    "Only your selected account data is shown.": "फक्त निवडलेल्या खात्याचा डेटा दाखवला जातो.",
    "Offline ready": "ऑफलाइन तयार",
    "Supported changes can wait safely for sync.": "समर्थित बदल सुरक्षितपणे सिंकची वाट पाहू शकतात.",
    "Report friendly": "अहवालांसाठी तयार",
    "Every approved input feeds your charts.": "प्रत्येक मंजूर इनपुट तुमच्या चार्टमध्ये जातो.",
    "Welcome back": "पुन्हा स्वागत आहे",
    "Start your workspace": "तुमचे कार्यक्षेत्र सुरू करा",
    "Account recovery": "खाते पुनर्प्राप्ती",
    "Password reset": "पासवर्ड रीसेट",
    "Sign in to NovaCent": "NovaCent मध्ये साइन इन करा",
    "Working on your request...": "तुमची विनंती पूर्ण होत आहे...",
    "Processing statement...": "स्टेटमेंट प्रक्रिया होत आहे...",
    "Saving imported rows...": "आयात ओळी जतन होत आहेत...",
    "Updating imported rows...": "आयात ओळी अपडेट होत आहेत...",
    "Syncing changes...": "बदल सिंक होत आहेत...",
    "Updating notifications...": "सूचना अपडेट होत आहेत...",
    "Signing out...": "साइन आउट होत आहे...",
    "Please wait while NovaCent finishes this request.": "कृपया थांबा, NovaCent ही विनंती पूर्ण करत आहे.",
    manual: "मॅन्युअल",
    import: "आयात",
    recurring: "आवर्ती",
    party: "समूह",
    trip: "प्रवास",
    settlement: "सेटलमेंट",
    "More details": "अधिक तपशील",
    "This statement is password protected. Enter the statement password and upload again.": "हे स्टेटमेंट पासवर्डने सुरक्षित आहे. पासवर्ड टाकून पुन्हा अपलोड करा.",
    "The statement password is incorrect. Please retry with the correct password.": "स्टेटमेंट पासवर्ड चुकीचा आहे. योग्य पासवर्डसह पुन्हा प्रयत्न करा.",
    "Track limits, spot overspend early, and keep categories honest.": "मर्यादा ट्रॅक करा, जास्त खर्च लवकर ओळखा आणि श्रेणी अचूक ठेवा.",
    "Create recurring rules that post due expenses automatically in the background.": "देय खर्च पार्श्वभूमीत आपोआप पोस्ट करणारे आवर्ती नियम तयार करा.",
    "New recurring rule": "नवीन आवर्ती नियम",
    "Edit recurring rule": "आवर्ती नियम संपादित करा",
    Description: "वर्णन",
    Frequency: "वारंवारता",
    Daily: "दररोज",
    Weekly: "दर आठवड्याला",
    Interval: "मध्यांतर",
    "Start date": "सुरू तारीख",
    "End date": "शेवटची तारीख",
    Notes: "नोट्स",
    "Save recurring rule": "आवर्ती नियम जतन करा",
    "Update recurring rule": "आवर्ती नियम अपडेट करा",
    "Recurring rule saved.": "आवर्ती नियम जतन झाला.",
    "Unable to save recurring rule.": "आवर्ती नियम जतन करता आला नाही.",
    "Recurring rule updated.": "आवर्ती नियम अपडेट झाला.",
    "Unable to update recurring rule.": "आवर्ती नियम अपडेट करता आला नाही.",
    "Recurring rules": "आवर्ती नियम",
    "Next run": "पुढील रन",
    Edit: "संपादित करा",
    Pause: "थांबवा",
    Resume: "पुन्हा सुरू करा",
    End: "समाप्त",
    active: "सक्रिय",
    paused: "थांबलेले",
    ended: "समाप्त",
    daily: "दररोज",
    weekly: "दर आठवड्याला",
    monthly: "मासिक",
    yearly: "वार्षिक",
    "No recurring rules yet": "अजून कोणतेही आवर्ती नियम नाहीत",
    "Create rules for expenses that should post automatically when due.": "देय झाल्यावर आपोआप पोस्ट होणाऱ्या खर्चांसाठी नियम तयार करा.",
    "Jan 1 to Dec 31": "1 जानेवारी ते 31 डिसेंबर",
    "1st of this month to month end": "या महिन्याच्या 1 तारखेपासून महिन्याच्या शेवटपर्यंत",
    Percentages: "टक्केवारी",
    Shares: "शेअर्स",
    shares: "शेअर्स",
    "Create party from selection": "निवडीतून समूह तयार करा",
    "Add selected expenses to split": "निवडलेले खर्च वाटणीत जोडा",
    "Existing party": "विद्यमान समूह",
    "Choose party": "समूह निवडा",
    "Add to party split": "समूह वाटणीत जोडा",
    "Create new party instead": "नवीन समूह तयार करा",
    "Add staged expenses": "तयार खर्च जोडा",
    "Add at least one more participant before adding staged expenses.": "तयार खर्च जोडण्यापूर्वी किमान अजून एक सहभागी जोडा.",
    "Some selected expenses cannot be added to a split.": "काही निवडलेले खर्च वाटणीत जोडता येत नाहीत.",
    "This is used for new entries. Existing expenses keep the currency they were saved with.": "हे नवीन नोंदींसाठी वापरले जाते. आधीचे खर्च ज्या चलनात सेव झाले आहेत त्याच चलनात राहतील.",
    Investment: "गुंतवणूक",
    "Mutual Funds": "म्युच्युअल फंड",
    Stocks: "स्टॉक",
    "Postal Investments": "पोस्टल गुंतवणूक",
    "Insurance Investments": "विमा गुंतवणूक",
    "PPF/NPS": "PPF/NPS",
    "Fixed Deposits": "फिक्स्ड डिपॉझिट",
    "Recurring Deposits": "रिकरिंग डिपॉझिट",
    Bonds: "बॉन्ड",
    Gold: "सोने",
    Crypto: "क्रिप्टो",
    "Other Investments": "इतर गुंतवणूक",
    Type: "प्रकार",
    Spend: "खर्च",
    Income: "उत्पन्न",
    Transfer: "ट्रान्सफर",
    View: "पहा",
    "All transactions": "सर्व व्यवहार",
    "Actual spends only": "फक्त प्रत्यक्ष खर्च",
    Profile: "प्रोफाइल",
    "Open profile": "प्रोफाइल उघडा",
    "View your account details, update your display name, and manage password access.": "तुमचे खाते तपशील पहा, दिसणारे नाव अपडेट करा आणि पासवर्ड प्रवेश सांभाळा.",
    "Profile details": "प्रोफाइल तपशील",
    "Full name": "पूर्ण नाव",
    "Sign-in method": "साइन-इन पद्धत",
    "Email and password": "ईमेल आणि पासवर्ड",
    "Default account": "डीफॉल्ट खाते",
    "Primary Account": "प्राथमिक खाते",
    "Member since": "सदस्यत्व सुरू",
    "Last updated": "शेवटचे अपडेट",
    "User ID": "यूजर ID",
    "Not available": "उपलब्ध नाही",
    "Profile identifiers are read-only and managed by your account.": "प्रोफाइल ओळख तपशील फक्त वाचण्यासाठी आहेत आणि तुमच्या खात्यातून व्यवस्थापित होतात.",
    "Edit name": "नाव संपादित करा",
    "This is the name friends see in parties, splits, and shared activity.": "हेच नाव मित्रांना समूह, वाटणी आणि शेअर केलेल्या क्रियेत दिसते.",
    "Save name": "नाव जतन करा",
    "Name updated.": "नाव अपडेट झाले.",
    "Unable to update profile.": "प्रोफाइल अपडेट करता आली नाही.",
    "Change password": "पासवर्ड बदला",
    "Current password": "सध्याचा पासवर्ड",
    "Confirm new password": "नवीन पासवर्ड निश्चित करा",
    "Use at least 8 characters for the new password.": "नवीन पासवर्डसाठी किमान 8 अक्षरे वापरा.",
    "Password changed.": "पासवर्ड बदलला.",
    "Passwords do not match.": "पासवर्ड जुळत नाहीत.",
    "Unable to change password.": "पासवर्ड बदलता आला नाही.",
    "Password change is only available for email/password accounts.": "पासवर्ड बदलणे फक्त ईमेल/पासवर्ड खात्यांसाठी उपलब्ध आहे.",
    "Current password is incorrect.": "सध्याचा पासवर्ड चुकीचा आहे.",
    "Loading profile": "प्रोफाइल लोड होत आहे",
    "Merchant or category": "व्यापारी किंवा श्रेणी",
    "Expense ledger with merchant, category, amount, and status": "व्यापारी, श्रेणी, रक्कम आणि स्थितीसह खर्च वही",
    Timeframe: "कालावधी",
    "Investment ledger": "गुंतवणूक लेजर",
    "Filtered investment total": "फिल्टर केलेल्या गुंतवणुकीची एकूण रक्कम",
    "Filtered total": "फिल्टर केलेली एकूण रक्कम",
    "filtered transactions": "फिल्टर केलेले व्यवहार",
    "Loading investments": "गुंतवणूक लोड होत आहे",
    "No investments yet": "अजून कोणतीही गुंतवणूक नाही",
    "Investment transactions will appear here once added or imported.": "जोडलेली किंवा आयात केलेली गुंतवणूक येथे दिसेल.",
    "Expense classification updated.": "खर्च वर्गीकरण अपडेट झाले.",
    "Unable to update expense classification.": "खर्च वर्गीकरण अपडेट करता आले नाही.",
    "Investment classification updated.": "गुंतवणूक वर्गीकरण अपडेट झाले.",
    "Unable to update investment classification.": "गुंतवणूक वर्गीकरण अपडेट करता आले नाही.",
    "Review investment transactions separately from day-to-day spending.": "गुंतवणूक व्यवहार रोजच्या खर्चापासून वेगळे पहा.",
    "Invested this month": "या महिन्यात गुंतवणूक",
    "Invested this year": "या वर्षी गुंतवणूक",
    "Investment activity in this timeframe": "या कालावधीतील गुंतवणूक",
    "Investments by month": "महिन्यानुसार गुंतवणूक",
    "Investment trend chart": "गुंतवणूक ट्रेंड चार्ट",
    "Customer support": "ग्राहक सहायता",
    "Send feature requests, issue reports, or praise to the NovaCent team.": "NovaCent टीमला फीचर विनंती, समस्या अहवाल किंवा कौतुक पाठवा.",
    Name: "नाव",
    "Request type": "विनंती प्रकार",
    "Add feature": "फीचर जोडा",
    "Report issue": "समस्या नोंदवा",
    Praise: "कौतुक",
    Comments: "टिप्पण्या",
    "Send support request": "सहाय्य विनंती पाठवा"
  }
};

export type GuideContent = {
  pageTitle: string;
  pageDescription: string;
  settingsDescription: string;
  backToSettings: string;
  openGuide: string;
  quickStartTitle: string;
  quickStartAria: string;
  sectionsAria: string;
  featureOverviewTitle: string;
  featureOverviewAria: string;
  featureOverview: Array<{ title: string; description: string }>;
  quickStart: Array<{ title: string; description: string }>;
  sections: Array<{ title: string; description: string; steps: string[] }>;
};

export const guideContent: Record<Language, GuideContent> = {
  en: {
    pageTitle: "How to use NovaCent",
    pageDescription: "A first-time walkthrough for setting up your workspace, adding expenses, separating investments, importing statements, reviewing budgets, and reading reports.",
    settingsDescription: "A simple new-user guide for profile, accounts, daily expenses, investments, statement imports, budgets, parties, recurring rules, reports, notifications, and offline use.",
    backToSettings: "Back to settings",
    openGuide: "Open guide",
    quickStartTitle: "Quick start",
    quickStartAria: "Quick start checklist",
    sectionsAria: "NovaCent guide sections",
    featureOverviewTitle: "What you can do with NovaCent",
    featureOverviewAria: "NovaCent feature overview",
    featureOverview: [
      { title: "Track expenses", description: "Add, review, search, and manage your daily spending in one place." },
      { title: "Import bank statements", description: "Upload statements and review extracted transactions before saving them." },
      { title: "Manage budgets", description: "Set monthly or yearly budgets and see how much you have used." },
      { title: "Track investments", description: "Keep investment transactions separate from daily spend and budget usage." },
      { title: "View reports", description: "Analyze spending, cash flow, categories, merchants, budgets, and parties visually." },
      { title: "Split shared expenses", description: "Create parties, add friends, split costs, and track settlements." },
      { title: "Handle recurring expenses", description: "Set up repeating expenses like rent, subscriptions, or EMIs." },
      { title: "Use multiple currencies", description: "Save expenses in different currencies while keeping reports consistent." },
      { title: "Work offline", description: "Add supported changes offline and sync them when you are back online." },
      { title: "Get notifications", description: "Stay informed about imports, budget alerts, settlements, and important actions." },
      { title: "Manage your profile", description: "Review profile details, update your display name, or change your password." },
      { title: "Customize preferences", description: "Change language, theme, default currency, and account settings." },
      { title: "Install as an app", description: "Use NovaCent like a mobile or desktop app with PWA support." },
      { title: "Request support", description: "Send feedback, report issues, or suggest new features from inside the app." }
    ],
    quickStart: [
      {
        title: "Choose the right account",
        description: "Use the account switcher before entering anything, because every expense, budget, import, party, and report is account-scoped."
      },
      {
        title: "Record or import spending",
        description: "Add expenses manually for daily spending, or upload a statement and approve only the rows you want to save."
      },
      {
        title: "Review budgets and reports",
        description: "Use budgets for limits and reports for patterns. Shared party settlements are reflected in your final personal spend."
      }
    ],
    sections: [
      {
        title: "Start with your account workspace",
        description: "NovaCent keeps every account separate, so the first thing to check is the account switcher at the top or in the mobile menu.",
        steps: [
          "Pick the account you want to work in before adding expenses, budgets, parties, or imports.",
          "Use the language and theme controls whenever you want a different reading experience.",
          "Open Hi <name> in the navigation footer to review your profile, update your display name, or change your password.",
          "Watch the online status. If you are offline, NovaCent keeps supported changes locally and syncs them when you are back online.",
          "If a blurred progress screen appears, wait for NovaCent to finish saving, syncing, or processing before continuing."
        ]
      },
      {
        title: "Add daily expenses",
        description: "Use Expenses for quick manual entries and for reviewing what is already in your ledger.",
        steps: [
          "Open Expenses and use Quick add at the top of the page.",
          "Enter merchant, category, amount, currency, and date. The date defaults to today.",
          "Use search to find older entries by merchant or category.",
          "Use the View filter to switch between all transactions and actual spends only.",
          "If a row is classified incorrectly, change its category or money-flow type from the row dropdowns.",
          "Use row checkboxes to select expenses, then delete selected unlocked expenses in one confirmed action.",
          "Delete unlocked expenses only after confirming. Settlement rows and settled party expenses stay protected."
        ]
      },
      {
        title: "Separate investments from spending",
        description: "Investments are tracked away from day-to-day spend so budgets and totals stay practical.",
        steps: [
          "Open Investments to review rows tagged as stocks, mutual funds, insurance investments, postal investments, or other investments.",
          "Use 1 month, 3 months, 1 year, 3 years, All, or a custom date range to narrow the list.",
          "Check the filtered total to see the investment amount for the selected period.",
          "If a statement row was tagged wrongly, update the category or money-flow type from the ledger dropdowns."
        ]
      },
      {
        title: "Track budgets",
        description: "Budgets help you see whether this month or year is still under control.",
        steps: [
          "Create a monthly or yearly budget and choose whether it tracks overall spend or a single category.",
          "Monthly budgets run from the 1st of the current month to month end. Yearly budgets run from Jan 1 to Dec 31.",
          "Expand a budget to see exactly which expenses are included in its calculation.",
          "Set an alert percentage if you want a custom warning point. The default is 80%."
        ]
      },
      {
        title: "Import bank statements",
        description: "Use Import review when you want NovaCent to read statement rows before saving them to your ledger.",
        steps: [
          "Upload a supported statement file. If the PDF is locked, enter the statement password before upload.",
          "Review extracted rows before approval. Nothing is saved as an expense until you approve it.",
          "Choose a category from the dropdown or leave the row Uncategorized.",
          "Use the possible duplicates filter when another statement may contain rows you already imported.",
          "Approve valid rows one by one, approve all safe rows at once, approve everything except possible duplicates, or delete all staged rows if the import is wrong."
        ]
      },
      {
        title: "Share expenses with parties",
        description: "Parties are for shared expenses with registered friends or non-registered placeholders.",
        steps: [
          "Create a party with a clear name, then open it.",
          "Search for a friend by name or email. If no registered user exists, add the entered name as a non-registered person.",
          "Add party expenses and choose even, manual amount, percentage, or share-based split.",
          "Registered participants request settlement approval. Non-registered placeholders settle directly.",
          "Converted party expenses still affect the overall ledger according to the final personal spend."
        ]
      },
      {
        title: "Automate recurring expenses",
        description: "Recurring rules are for bills or expenses that should post automatically in the background.",
        steps: [
          "Create a recurring rule with merchant, category, amount, currency, frequency, start date, and optional end date.",
          "Pause, resume, edit, or end a rule when the real-world bill changes.",
          "Recurring expenses feed the normal ledger, budgets, and reports once they are posted."
        ]
      },
      {
        title: "Read reports",
        description: "Reports turn the ledger into charts for spend, cash flow, budget variance, merchants, parties, and currencies.",
        steps: [
          "Use the timeframe selector for past 15 days, 1 month, 3 months, 1 year, 3 years, All, or a custom date range.",
          "Check Spend by category to understand where money is going.",
          "Use the investment report to review investment activity separately from normal spend.",
          "Use Cash flow trend to compare income, spend, and saved amount over time.",
          "Export CSV for spreadsheet work or use Export PDF for a printable report."
        ]
      },
      {
        title: "Keep an eye on notifications and support",
        description: "Notifications collect budget alerts, imports, settlement requests, and other important updates.",
        steps: [
          "Open the bell icon to review unread notifications.",
          "Use Mark read when you have reviewed them, or Clear to remove old notifications.",
          "Use Support to send feature requests, issue reports, or praise to the NovaCent team."
        ]
      }
    ]
  },
  es: {
    pageTitle: "Como usar NovaCent",
    pageDescription: "Una guia inicial para configurar tu espacio, agregar gastos, separar inversiones, importar extractos, revisar presupuestos y leer informes.",
    settingsDescription: "Una guia sencilla para nuevos usuarios sobre perfil, cuentas, gastos diarios, inversiones, importacion de extractos, presupuestos, grupos, reglas recurrentes, informes, notificaciones y uso sin conexion.",
    backToSettings: "Volver a ajustes",
    openGuide: "Abrir guia",
    quickStartTitle: "Inicio rapido",
    quickStartAria: "Lista de inicio rapido",
    sectionsAria: "Secciones de guia de NovaCent",
    featureOverviewTitle: "Que puedes hacer con NovaCent",
    featureOverviewAria: "Resumen de funciones de NovaCent",
    featureOverview: [
      { title: "Controlar gastos", description: "Agrega, revisa, busca y gestiona tus gastos diarios en un solo lugar." },
      { title: "Importar extractos bancarios", description: "Sube extractos y revisa las transacciones extraidas antes de guardarlas." },
      { title: "Gestionar presupuestos", description: "Crea presupuestos mensuales o anuales y mira cuanto has usado." },
      { title: "Controlar inversiones", description: "Mantiene inversiones separadas de gastos diarios y presupuestos." },
      { title: "Ver informes", description: "Analiza gastos, flujo de caja, categorias, comercios, presupuestos y grupos visualmente." },
      { title: "Dividir gastos compartidos", description: "Crea grupos, agrega amigos, divide costes y controla liquidaciones." },
      { title: "Gestionar gastos recurrentes", description: "Configura gastos repetidos como alquiler, suscripciones o EMIs." },
      { title: "Usar varias monedas", description: "Guarda gastos en distintas monedas y mantiene informes consistentes." },
      { title: "Trabajar sin conexion", description: "Agrega cambios admitidos sin conexion y sincronizalos al volver." },
      { title: "Recibir notificaciones", description: "Mantente informado sobre importaciones, alertas de presupuesto, liquidaciones y acciones importantes." },
      { title: "Gestionar tu perfil", description: "Revisa tus datos, actualiza tu nombre visible o cambia tu contrasena." },
      { title: "Personalizar preferencias", description: "Cambia idioma, tema, moneda predeterminada y ajustes de cuenta." },
      { title: "Instalar como app", description: "Usa NovaCent como app movil o de escritorio con soporte PWA." },
      { title: "Solicitar soporte", description: "Envia comentarios, reporta problemas o sugiere funciones desde la app." }
    ],
    quickStart: [
      {
        title: "Elige la cuenta correcta",
        description: "Usa el selector de cuenta antes de ingresar datos, porque cada gasto, presupuesto, importacion, grupo e informe pertenece a una cuenta."
      },
      {
        title: "Registra o importa gastos",
        description: "Agrega gastos manuales para el dia a dia, o sube un extracto y aprueba solo las filas que quieras guardar."
      },
      {
        title: "Revisa presupuestos e informes",
        description: "Usa presupuestos para limites e informes para patrones. Los acuerdos de grupos se reflejan en tu gasto personal final."
      }
    ],
    sections: [
      {
        title: "Empieza con tu espacio de cuenta",
        description: "NovaCent mantiene cada cuenta separada, asi que primero revisa el selector de cuenta arriba o en el menu movil.",
        steps: [
          "Elige la cuenta en la que quieres trabajar antes de agregar gastos, presupuestos, grupos o importaciones.",
          "Usa los controles de idioma y tema cuando quieras una experiencia de lectura diferente.",
          "Abre Hola <nombre> en el pie de navegacion para revisar tu perfil, actualizar tu nombre visible o cambiar tu contrasena.",
          "Mira el estado en linea. Si estas sin conexion, NovaCent guarda los cambios admitidos localmente y los sincroniza al volver.",
          "Si aparece una pantalla borrosa de progreso, espera a que NovaCent termine de guardar, sincronizar o procesar antes de continuar."
        ]
      },
      {
        title: "Agrega gastos diarios",
        description: "Usa Gastos para entradas manuales rapidas y para revisar lo que ya esta en tu libro.",
        steps: [
          "Abre Gastos y usa Agregar rapido en la parte superior.",
          "Ingresa comercio, categoria, importe, moneda y fecha. La fecha se establece en hoy por defecto.",
          "Usa la busqueda para encontrar entradas antiguas por comercio o categoria.",
          "Usa el filtro Ver para alternar entre todas las transacciones y solo gastos reales.",
          "Si una fila esta clasificada incorrectamente, cambia la categoria o el tipo de flujo desde los desplegables.",
          "Usa las casillas para seleccionar gastos y eliminar los gastos desbloqueados seleccionados en una sola accion confirmada.",
          "Elimina gastos desbloqueados solo despues de confirmar. Los acuerdos y gastos de grupo saldados quedan protegidos."
        ]
      },
      {
        title: "Separa inversiones del gasto",
        description: "Las inversiones se controlan aparte del gasto diario para que presupuestos y totales sigan siendo utiles.",
        steps: [
          "Abre Inversiones para revisar filas marcadas como acciones, fondos mutuos, inversiones de seguro, inversiones postales u otras inversiones.",
          "Usa 1 mes, 3 meses, 1 ano, 3 anos, Todo o un rango personalizado para acotar la lista.",
          "Consulta el total filtrado para ver el monto invertido en el periodo seleccionado.",
          "Si una fila del extracto quedo mal marcada, actualiza la categoria o el tipo de flujo desde los desplegables del libro."
        ]
      },
      {
        title: "Controla presupuestos",
        description: "Los presupuestos te ayudan a ver si el mes o el ano siguen bajo control.",
        steps: [
          "Crea un presupuesto mensual o anual y elige si controla el gasto total o una sola categoria.",
          "Los presupuestos mensuales van del dia 1 al fin del mes. Los anuales van del 1 de enero al 31 de diciembre.",
          "Expande un presupuesto para ver exactamente que gastos entran en el calculo.",
          "Define un porcentaje de alerta si quieres un aviso personalizado. El valor predeterminado es 80%."
        ]
      },
      {
        title: "Importa extractos bancarios",
        description: "Usa Revision de importacion cuando quieras que NovaCent lea filas del extracto antes de guardarlas.",
        steps: [
          "Sube un archivo admitido. Si el PDF esta bloqueado, ingresa la contrasena antes de subirlo.",
          "Revisa las filas extraidas antes de aprobarlas. Nada se guarda como gasto hasta que lo apruebes.",
          "Elige una categoria en el desplegable o deja la fila como Sin categoria.",
          "Usa el filtro de posibles duplicados si otro extracto puede contener filas ya importadas.",
          "Aprueba filas validas una por una, aprueba todas las filas seguras, aprueba todo excepto posibles duplicados o elimina todas las filas preparadas si la importacion es incorrecta."
        ]
      },
      {
        title: "Comparte gastos con grupos",
        description: "Los grupos sirven para gastos compartidos con amigos registrados o personas no registradas.",
        steps: [
          "Crea un grupo con un nombre claro y luego abrelo.",
          "Busca un amigo por nombre o email. Si no existe un usuario registrado, agrega el nombre como persona no registrada.",
          "Agrega gastos de grupo y elige division igual, monto manual, porcentaje o partes.",
          "Los participantes registrados solicitan aprobacion de liquidacion. Las personas no registradas se saldan directamente.",
          "Los gastos convertidos de grupo siguen afectando el libro general segun tu gasto personal final."
        ]
      },
      {
        title: "Automatiza gastos recurrentes",
        description: "Las reglas recurrentes son para facturas o gastos que deben publicarse automaticamente en segundo plano.",
        steps: [
          "Crea una regla con comercio, categoria, importe, moneda, frecuencia, fecha inicial y fecha final opcional.",
          "Pausa, reanuda, edita o finaliza una regla cuando cambie la factura real.",
          "Los gastos recurrentes alimentan el libro, presupuestos e informes cuando se publican."
        ]
      },
      {
        title: "Lee informes",
        description: "Los informes convierten el libro en graficos de gasto, flujo de caja, presupuesto, comercios, grupos y monedas.",
        steps: [
          "Usa el selector de periodo para 15 dias, 1 mes, 3 meses, 1 ano, 3 anos, Todo o un rango personalizado.",
          "Revisa Gasto por categoria para entender adonde va el dinero.",
          "Usa el informe de inversiones para revisar la actividad de inversion separada del gasto normal.",
          "Usa Flujo de caja para comparar ingresos, gastos y ahorro en el tiempo.",
          "Exporta CSV para trabajar en hojas de calculo o PDF para un informe imprimible."
        ]
      },
      {
        title: "Revisa notificaciones y soporte",
        description: "Las notificaciones reunen alertas de presupuesto, importaciones, liquidaciones y avisos importantes.",
        steps: [
          "Abre el icono de campana para revisar notificaciones no leidas.",
          "Usa Marcar leidas cuando las hayas revisado, o Borrar para quitar notificaciones antiguas.",
          "Usa Soporte para enviar solicitudes de funciones, reportes de problemas o elogios al equipo de NovaCent."
        ]
      }
    ]
  },
  fr: {
    pageTitle: "Comment utiliser NovaCent",
    pageDescription: "Un guide de premiere utilisation pour configurer l'espace, ajouter des depenses, separer les investissements, importer des releves, verifier les budgets et lire les rapports.",
    settingsDescription: "Un guide simple pour nouveaux utilisateurs sur profil, comptes, depenses quotidiennes, investissements, imports de releves, budgets, groupes, regles recurrentes, rapports, notifications et usage hors ligne.",
    backToSettings: "Retour aux parametres",
    openGuide: "Ouvrir le guide",
    quickStartTitle: "Demarrage rapide",
    quickStartAria: "Liste de demarrage rapide",
    sectionsAria: "Sections du guide NovaCent",
    featureOverviewTitle: "Ce que vous pouvez faire avec NovaCent",
    featureOverviewAria: "Apercu des fonctionnalites NovaCent",
    featureOverview: [
      { title: "Suivre les depenses", description: "Ajoutez, verifiez, recherchez et gerez vos depenses quotidiennes au meme endroit." },
      { title: "Importer des releves bancaires", description: "Importez des releves et verifiez les transactions extraites avant enregistrement." },
      { title: "Gerer les budgets", description: "Creez des budgets mensuels ou annuels et voyez ce qui est deja utilise." },
      { title: "Suivre les investissements", description: "Gardez les investissements separes des depenses quotidiennes et des budgets." },
      { title: "Voir les rapports", description: "Analysez visuellement depenses, tresorerie, categories, marchands, budgets et groupes." },
      { title: "Partager les depenses", description: "Creez des groupes, ajoutez des amis, partagez les couts et suivez les reglements." },
      { title: "Gerer les depenses recurrentes", description: "Configurez les depenses repetees comme loyer, abonnements ou mensualites." },
      { title: "Utiliser plusieurs devises", description: "Enregistrez les depenses dans plusieurs devises tout en gardant des rapports coherents." },
      { title: "Travailler hors ligne", description: "Ajoutez les changements pris en charge hors ligne puis synchronisez-les au retour." },
      { title: "Recevoir des notifications", description: "Restez informe des imports, alertes de budget, reglements et actions importantes." },
      { title: "Gerer le profil", description: "Consultez vos informations, modifiez le nom affiche ou changez le mot de passe." },
      { title: "Personnaliser les preferences", description: "Changez langue, theme, devise par defaut et parametres de compte." },
      { title: "Installer comme application", description: "Utilisez NovaCent comme application mobile ou bureau avec le support PWA." },
      { title: "Demander de l'aide", description: "Envoyez commentaires, problemes ou idees de fonctionnalites depuis l'application." }
    ],
    quickStart: [
      {
        title: "Choisir le bon compte",
        description: "Utilisez le selecteur de compte avant de saisir quoi que ce soit, car chaque depense, budget, import, groupe et rapport est lie a un compte."
      },
      {
        title: "Saisir ou importer les depenses",
        description: "Ajoutez les depenses du quotidien manuellement, ou importez un releve et approuvez seulement les lignes a conserver."
      },
      {
        title: "Verifier budgets et rapports",
        description: "Utilisez les budgets pour les limites et les rapports pour les tendances. Les reglements de groupe se refletent dans votre depense personnelle finale."
      }
    ],
    sections: [
      {
        title: "Commencer par l'espace du compte",
        description: "NovaCent garde chaque compte separe, donc verifiez d'abord le selecteur de compte en haut ou dans le menu mobile.",
        steps: [
          "Choisissez le compte a utiliser avant d'ajouter des depenses, budgets, groupes ou imports.",
          "Utilisez les controles de langue et de theme pour changer votre experience de lecture.",
          "Ouvrez Bonjour <nom> dans le pied de navigation pour consulter le profil, modifier le nom affiche ou changer le mot de passe.",
          "Surveillez le statut en ligne. Hors ligne, NovaCent garde les changements pris en charge localement puis les synchronise.",
          "Si un ecran de progression floute apparait, attendez que NovaCent termine l'enregistrement, la synchronisation ou le traitement."
        ]
      },
      {
        title: "Ajouter les depenses quotidiennes",
        description: "Utilisez Depenses pour les saisies rapides et pour verifier le grand livre existant.",
        steps: [
          "Ouvrez Depenses et utilisez l'ajout rapide en haut de page.",
          "Saisissez marchand, categorie, montant, devise et date. La date est aujourd'hui par defaut.",
          "Utilisez la recherche pour retrouver des lignes par marchand ou categorie.",
          "Utilisez le filtre Vue pour alterner entre toutes les transactions et les depenses reelles seulement.",
          "Si une ligne est mal classee, changez sa categorie ou son type de flux depuis les listes de la ligne.",
          "Utilisez les cases a cocher pour selectionner des depenses, puis supprimer les depenses deverrouillees selectionnees en une seule action confirmee.",
          "Supprimez les depenses deverrouillees seulement apres confirmation. Les reglements et depenses de groupe reglees restent proteges."
        ]
      },
      {
        title: "Separer les investissements des depenses",
        description: "Les investissements sont suivis a part du quotidien pour garder budgets et totaux utiles.",
        steps: [
          "Ouvrez Investissements pour verifier les lignes marquees actions, fonds mutuels, investissements assurance, investissements postaux ou autres investissements.",
          "Utilisez 1 mois, 3 mois, 1 an, 3 ans, Tout ou une plage personnalisee pour reduire la liste.",
          "Consultez le total filtre pour voir le montant investi sur la periode choisie.",
          "Si une ligne de releve est mal marquee, mettez a jour la categorie ou le type de flux depuis les listes du grand livre."
        ]
      },
      {
        title: "Suivre les budgets",
        description: "Les budgets montrent si le mois ou l'annee restent sous controle.",
        steps: [
          "Creez un budget mensuel ou annuel et choisissez le suivi global ou une categorie.",
          "Les budgets mensuels vont du 1er au dernier jour du mois. Les budgets annuels vont du 1er janvier au 31 decembre.",
          "Depliez un budget pour voir les depenses incluses dans le calcul.",
          "Definissez un pourcentage d'alerte personnalise si besoin. La valeur par defaut est 80%."
        ]
      },
      {
        title: "Importer les releves bancaires",
        description: "Utilisez Revue d'import pour faire lire les lignes du releve par NovaCent avant enregistrement.",
        steps: [
          "Importez un fichier pris en charge. Si le PDF est verrouille, saisissez le mot de passe avant l'import.",
          "Verifiez les lignes extraites avant approbation. Rien n'est enregistre comme depense avant votre accord.",
          "Choisissez une categorie ou laissez la ligne Non classee.",
          "Utilisez le filtre de doublons possibles si un autre releve peut contenir les memes lignes.",
          "Approuvez les lignes valides une par une, approuvez toutes les lignes sures, approuvez tout sauf les doublons possibles ou supprimez toutes les lignes preparees si l'import est incorrect."
        ]
      },
      {
        title: "Partager des depenses avec des groupes",
        description: "Les groupes servent aux depenses partagees avec des amis inscrits ou des participants externes.",
        steps: [
          "Creez un groupe avec un nom clair, puis ouvrez-le.",
          "Recherchez un ami par nom ou email. Si aucun utilisateur inscrit n'existe, ajoutez le nom comme participant externe.",
          "Ajoutez des depenses de groupe et choisissez partage egal, montant manuel, pourcentage ou parts.",
          "Les participants inscrits demandent une approbation de reglement. Les externes sont regles directement.",
          "Les depenses de groupe converties continuent d'affecter le grand livre selon votre depense personnelle finale."
        ]
      },
      {
        title: "Automatiser les depenses recurrentes",
        description: "Les regles recurrentes servent aux factures ou depenses qui doivent se publier automatiquement.",
        steps: [
          "Creez une regle avec marchand, categorie, montant, devise, frequence, date de debut et date de fin facultative.",
          "Suspendez, reprenez, modifiez ou terminez une regle lorsque la facture reelle change.",
          "Les depenses recurrentes alimentent le grand livre, les budgets et les rapports une fois publiees."
        ]
      },
      {
        title: "Lire les rapports",
        description: "Les rapports transforment le grand livre en graphiques de depenses, tresorerie, budgets, marchands, groupes et devises.",
        steps: [
          "Utilisez le selecteur de periode pour 15 jours, 1 mois, 3 mois, 1 an, 3 ans, Tout ou une plage personnalisee.",
          "Consultez Depenses par categorie pour comprendre ou part l'argent.",
          "Utilisez le rapport d'investissements pour verifier l'activite d'investissement separement des depenses normales.",
          "Utilisez Flux de tresorerie pour comparer revenus, depenses et epargne dans le temps.",
          "Exportez en CSV pour tableur ou en PDF pour un rapport imprimable."
        ]
      },
      {
        title: "Surveiller notifications et support",
        description: "Les notifications regroupent alertes de budget, imports, reglements et informations importantes.",
        steps: [
          "Ouvrez l'icone de cloche pour voir les notifications non lues.",
          "Utilisez Marquer comme lu apres lecture, ou Effacer pour retirer les anciennes notifications.",
          "Utilisez Support pour envoyer demandes de fonctionnalite, signalements ou compliments a l'equipe NovaCent."
        ]
      }
    ]
  },
  hi: {
    pageTitle: "NovaCent का उपयोग कैसे करें",
    pageDescription: "पहली बार उपयोग के लिए गाइड: कार्यक्षेत्र सेट करें, खर्च जोड़ें, निवेश अलग रखें, स्टेटमेंट आयात करें, बजट देखें और रिपोर्ट पढ़ें.",
    settingsDescription: "नए उपयोगकर्ता के लिए प्रोफ़ाइल, खाते, रोज के खर्च, निवेश, स्टेटमेंट आयात, बजट, समूह, आवर्ती नियम, रिपोर्ट, सूचनाएं और ऑफलाइन उपयोग की आसान गाइड.",
    backToSettings: "सेटिंग्स पर वापस जाएं",
    openGuide: "गाइड खोलें",
    quickStartTitle: "त्वरित शुरुआत",
    quickStartAria: "त्वरित शुरुआत सूची",
    sectionsAria: "NovaCent गाइड सेक्शन",
    featureOverviewTitle: "NovaCent में आप क्या कर सकते हैं",
    featureOverviewAria: "NovaCent सुविधाओं की झलक",
    featureOverview: [
      { title: "खर्च ट्रैक करें", description: "अपने रोज़ के खर्च एक ही जगह जोड़ें, देखें, खोजें और संभालें." },
      { title: "बैंक स्टेटमेंट आयात करें", description: "स्टेटमेंट अपलोड करें और सेव करने से पहले निकाली गई एंट्री देखें." },
      { title: "बजट संभालें", description: "मासिक या वार्षिक बजट सेट करें और देखें कितना उपयोग हुआ है." },
      { title: "निवेश ट्रैक करें", description: "निवेश को रोज़ के खर्च और बजट उपयोग से अलग रखें." },
      { title: "रिपोर्ट देखें", description: "खर्च, नकद प्रवाह, श्रेणियां, व्यापारी, बजट और समूह को चार्ट में समझें." },
      { title: "साझा खर्च बांटें", description: "समूह बनाएं, मित्र जोड़ें, खर्च बांटें और निपटारे ट्रैक करें." },
      { title: "आवर्ती खर्च संभालें", description: "किराया, सब्सक्रिप्शन या EMI जैसे दोहराए जाने वाले खर्च सेट करें." },
      { title: "कई मुद्राएं इस्तेमाल करें", description: "अलग-अलग मुद्राओं में खर्च सेव करें और रिपोर्ट को सुसंगत रखें." },
      { title: "ऑफलाइन काम करें", description: "समर्थित बदलाव ऑफलाइन जोड़ें और ऑनलाइन आने पर सिंक करें." },
      { title: "सूचनाएं पाएं", description: "आयात, बजट अलर्ट, निपटारे और जरूरी कार्रवाइयों की जानकारी पाएं." },
      { title: "प्रोफ़ाइल संभालें", description: "अपनी जानकारी देखें, नाम अपडेट करें या पासवर्ड बदलें." },
      { title: "पसंद बदलें", description: "भाषा, थीम, डिफ़ॉल्ट मुद्रा और खाता सेटिंग बदलें." },
      { title: "ऐप की तरह इंस्टॉल करें", description: "PWA सपोर्ट के साथ NovaCent को मोबाइल या डेस्कटॉप ऐप की तरह इस्तेमाल करें." },
      { title: "सहायता मांगें", description: "ऐप के अंदर से प्रतिक्रिया भेजें, समस्या बताएं या नई सुविधा सुझाएं." }
    ],
    quickStart: [
      {
        title: "सही खाता चुनें",
        description: "कुछ भी जोड़ने से पहले खाता स्विचर देखें, क्योंकि हर खर्च, बजट, आयात, समूह और रिपोर्ट किसी एक खाते से जुड़ी होती है."
      },
      {
        title: "खर्च जोड़ें या आयात करें",
        description: "रोज के खर्च मैनुअली जोड़ें, या स्टेटमेंट अपलोड करके केवल वही पंक्तियां मंज़ूर करें जिन्हें सहेजना है."
      },
      {
        title: "बजट और रिपोर्ट देखें",
        description: "सीमा के लिए बजट और खर्च के पैटर्न समझने के लिए रिपोर्ट इस्तेमाल करें. साझा समूह निपटान आपके अंतिम निजी खर्च में दिखते हैं."
      }
    ],
    sections: [
      {
        title: "अपने खाते के कार्यक्षेत्र से शुरू करें",
        description: "NovaCent हर खाते को अलग रखता है, इसलिए पहले ऊपर या मोबाइल मेनू में खाता बदलने वाला चयनक देखें.",
        steps: [
          "खर्च, बजट, समूह या आयात जोड़ने से पहले वह खाता चुनें जिसमें काम करना है.",
          "जब पढ़ने का अनुभव बदलना हो तो भाषा और थीम नियंत्रण इस्तेमाल करें.",
          "नेविगेशन के नीचे नमस्कार <नाम> खोलकर प्रोफ़ाइल देखें, नाम अपडेट करें या पासवर्ड बदलें.",
          "ऑनलाइन स्थिति देखें. ऑफलाइन होने पर NovaCent समर्थित बदलाव स्थानीय रूप से रखता है और ऑनलाइन होने पर समन्वय करता है.",
          "अगर धुंधली प्रगति स्क्रीन दिखे, तो आगे बढ़ने से पहले NovaCent को सेव, सिंक या प्रोसेस पूरा करने दें."
        ]
      },
      {
        title: "रोज के खर्च जोड़ें",
        description: "खर्च पेज त्वरित मैनुअल प्रविष्टियों और खाता-बही समीक्षा के लिए है.",
        steps: [
          "खर्च पेज खोलें और ऊपर त्वरित जोड़ का इस्तेमाल करें.",
          "व्यापारी, श्रेणी, राशि, मुद्रा और तारीख भरें. तारीख अपने आप आज की रहती है.",
          "पुरानी प्रविष्टियां व्यापारी या श्रेणी से खोजने के लिए खोज का इस्तेमाल करें.",
          "सभी लेनदेन और केवल वास्तविक खर्च के बीच बदलने के लिए दृश्य फ़िल्टर इस्तेमाल करें.",
          "अगर कोई पंक्ति गलत वर्गीकृत है, तो पंक्ति के ड्रॉपडाउन से श्रेणी या मनी-फ्लो प्रकार बदलें.",
          "चेकबॉक्स से खर्च चुनें और चुने हुए अनलॉक खर्चों को एक पुष्टि की गई कार्रवाई में हटाएं.",
          "लॉक नहीं किए गए खर्च पुष्टि के बाद ही हटाएं. निपटान पंक्तियां और निपटाए गए समूह खर्च सुरक्षित रहते हैं."
        ]
      },
      {
        title: "निवेश को खर्च से अलग रखें",
        description: "निवेश रोज़ के खर्च से अलग ट्रैक होते हैं ताकि बजट और कुल राशि उपयोगी रहें.",
        steps: [
          "स्टॉक्स, म्यूचुअल फंड, बीमा निवेश, डाक निवेश या अन्य निवेश वाली पंक्तियां देखने के लिए निवेश खोलें.",
          "सूची कम करने के लिए 1 महीना, 3 महीने, 1 साल, 3 साल, सभी या अपनी तारीख सीमा इस्तेमाल करें.",
          "चुनी गई अवधि में निवेश राशि देखने के लिए फ़िल्टर किया गया कुल देखें.",
          "अगर स्टेटमेंट पंक्ति गलत टैग हुई है, तो लेजर ड्रॉपडाउन से श्रेणी या मनी-फ्लो प्रकार बदलें."
        ]
      },
      {
        title: "बजट पर नज़र रखें",
        description: "बजट से पता चलता है कि महीना या साल नियंत्रण में है या नहीं.",
        steps: [
          "मासिक या वार्षिक बजट बनाएं और चुनें कि वह कुल खर्च पर नज़र रखेगा या किसी एक श्रेणी पर.",
          "मासिक बजट मौजूदा महीने की 1 तारीख से महीने के अंत तक चलता है. वार्षिक बजट 1 जनवरी से 31 दिसंबर तक चलता है.",
          "किस खर्च को गणना में लिया गया है, यह देखने के लिए बजट खोलकर देखें.",
          "अपनी चेतावनी सीमा के लिए अलर्ट प्रतिशत सेट करें. डिफ़ॉल्ट 80% है."
        ]
      },
      {
        title: "बैंक स्टेटमेंट आयात करें",
        description: "आयात समीक्षा तब इस्तेमाल करें जब NovaCent स्टेटमेंट पंक्तियों को खाता-बही में सहेजने से पहले पढ़े.",
        steps: [
          "समर्थित स्टेटमेंट फ़ाइल अपलोड करें. PDF लॉक हो तो अपलोड से पहले स्टेटमेंट पासवर्ड डालें.",
          "मंज़ूर करने से पहले निकाली गई पंक्तियों की समीक्षा करें. मंज़ूर किए बिना कोई पंक्ति खर्च नहीं बनती.",
          "ड्रॉपडाउन से श्रेणी चुनें या पंक्ति को अवर्गीकृत रहने दें.",
          "अगर दूसरे स्टेटमेंट में पहले से आयात की गई पंक्तियां हो सकती हैं, तो संभावित दोहराव फ़िल्टर इस्तेमाल करें.",
          "मान्य पंक्तियां एक-एक करके मंज़ूर करें, सभी सुरक्षित पंक्तियां मंज़ूर करें, संभावित दोहराव छोड़कर मंज़ूर करें, या आयात गलत हो तो सभी तैयार की गई पंक्तियां हटा दें."
        ]
      },
      {
        title: "समूहों में खर्च साझा करें",
        description: "समूह पंजीकृत दोस्तों या बिना पंजीकरण वाले अस्थायी प्रतिभागियों के साथ साझा खर्च के लिए हैं.",
        steps: [
          "स्पष्ट नाम के साथ समूह बनाएं और उसे खोलें.",
          "नाम या ईमेल से दोस्त खोजें. पंजीकृत उपयोगकर्ता न मिले तो डाला गया नाम बिना पंजीकरण वाले व्यक्ति की तरह जोड़ें.",
          "समूह खर्च जोड़ें और समान, मैनुअल राशि, प्रतिशत या हिस्सों पर आधारित विभाजन चुनें.",
          "पंजीकृत प्रतिभागी निपटान मंज़ूरी का अनुरोध करते हैं. बिना पंजीकरण वाले अस्थायी प्रतिभागी सीधे निपट जाते हैं.",
          "बदले गए समूह खर्च अंतिम निजी खर्च के हिसाब से पूरी खाता-बही को प्रभावित करते हैं."
        ]
      },
      {
        title: "आवर्ती खर्च स्वचालित करें",
        description: "आवर्ती नियम बिलों या ऐसे खर्चों के लिए हैं जो पृष्ठभूमि में अपने आप दर्ज होने चाहिए.",
        steps: [
          "व्यापारी, श्रेणी, राशि, मुद्रा, आवृत्ति, शुरू तारीख और वैकल्पिक अंत तारीख के साथ नियम बनाएं.",
          "वास्तविक बिल बदलने पर नियम को रोकें, फिर शुरू करें, संपादित करें या समाप्त करें.",
          "दर्ज होने के बाद आवर्ती खर्च सामान्य खाता-बही, बजट और रिपोर्ट में शामिल होते हैं."
        ]
      },
      {
        title: "रिपोर्ट पढ़ें",
        description: "रिपोर्ट खाता-बही को खर्च, नकदी प्रवाह, बजट अंतर, व्यापारियों, समूहों और मुद्राओं के चार्ट में बदलती हैं.",
        steps: [
          "पिछले 15 दिन, 1 महीना, 3 महीने, 1 साल, 3 साल, सभी या अपनी तारीख सीमा चुनने के लिए समयावधि चयनक इस्तेमाल करें.",
          "पैसा कहां जा रहा है समझने के लिए श्रेणी के अनुसार खर्च देखें.",
          "निवेश गतिविधि को सामान्य खर्च से अलग देखने के लिए निवेश रिपोर्ट इस्तेमाल करें.",
          "आय, खर्च और बचत की तुलना करने के लिए नकदी प्रवाह रुझान देखें.",
          "स्प्रेडशीट के लिए CSV निर्यात करें या छपाई योग्य रिपोर्ट के लिए PDF निर्यात करें."
        ]
      },
      {
        title: "सूचनाएं और सहायता देखें",
        description: "सूचनाएं बजट अलर्ट, आयात, निपटान अनुरोध और जरूरी अपडेट को एक जगह रखती हैं.",
        steps: [
          "अपठित सूचनाएं देखने के लिए घंटी चिह्न खोलें.",
          "समीक्षा के बाद पढ़ा हुआ मार्क करें इस्तेमाल करें, या पुरानी सूचनाएं हटाने के लिए साफ करें इस्तेमाल करें.",
          "सुविधा अनुरोध, समस्या रिपोर्ट या प्रशंसा भेजने के लिए सहायता इस्तेमाल करें."
        ]
      }
    ]
  },
  mr: {
    pageTitle: "NovaCent कसे वापरावे",
    pageDescription: "पहिल्यांदा वापरणाऱ्यांसाठी गाइड: कार्यक्षेत्र सेट करा, खर्च जोडा, गुंतवणूक वेगळी ठेवा, स्टेटमेंट आयात करा, बजेट तपासा आणि अहवाल वाचा.",
    settingsDescription: "नवीन वापरकर्त्यांसाठी प्रोफाइल, खाते, दररोजचे खर्च, गुंतवणूक, स्टेटमेंट आयात, बजेट, समूह, आवर्ती नियम, अहवाल, सूचना आणि ऑफलाइन वापराची सोपी गाइड.",
    backToSettings: "सेटिंग्जवर परत जा",
    openGuide: "गाइड उघडा",
    quickStartTitle: "जलद सुरुवात",
    quickStartAria: "जलद सुरुवात यादी",
    sectionsAria: "NovaCent गाइड विभाग",
    featureOverviewTitle: "NovaCent मध्ये तुम्ही काय करू शकता",
    featureOverviewAria: "NovaCent सुविधांचा आढावा",
    featureOverview: [
      { title: "खर्च ट्रॅक करा", description: "दररोजचे खर्च एकाच ठिकाणी जोडा, पाहा, शोधा आणि व्यवस्थापित करा." },
      { title: "बँक स्टेटमेंट आयात करा", description: "स्टेटमेंट अपलोड करा आणि सेव करण्यापूर्वी काढलेल्या नोंदी तपासा." },
      { title: "बजेट व्यवस्थापित करा", description: "मासिक किंवा वार्षिक बजेट सेट करा आणि किती वापरले ते पाहा." },
      { title: "गुंतवणूक ट्रॅक करा", description: "गुंतवणूक रोजच्या खर्च आणि बजेट वापरापासून वेगळी ठेवा." },
      { title: "अहवाल पाहा", description: "खर्च, रोकड प्रवाह, श्रेणी, व्यापारी, बजेट आणि समूह दृश्य स्वरूपात समजा." },
      { title: "सामायिक खर्च वाटा", description: "समूह तयार करा, मित्र जोडा, खर्च वाटा आणि निपटारे ट्रॅक करा." },
      { title: "आवर्ती खर्च हाताळा", description: "भाडे, सबस्क्रिप्शन किंवा EMI सारखे पुन्हा येणारे खर्च सेट करा." },
      { title: "अनेक चलने वापरा", description: "वेगवेगळ्या चलनात खर्च सेव करा आणि अहवाल सुसंगत ठेवा." },
      { title: "ऑफलाइन काम करा", description: "समर्थित बदल ऑफलाइन जोडा आणि ऑनलाइन झाल्यावर सिंक करा." },
      { title: "सूचना मिळवा", description: "आयात, बजेट अलर्ट, निपटारे आणि महत्त्वाच्या कृतींबद्दल माहिती मिळवा." },
      { title: "प्रोफाइल सांभाळा", description: "तुमची माहिती पहा, नाव अपडेट करा किंवा पासवर्ड बदला." },
      { title: "प्राधान्ये बदला", description: "भाषा, थीम, डीफॉल्ट चलन आणि खाते सेटिंग बदला." },
      { title: "अॅप म्हणून इंस्टॉल करा", description: "PWA सपोर्टसह NovaCent मोबाइल किंवा डेस्कटॉप अॅपसारखे वापरा." },
      { title: "सहाय्य मागा", description: "अॅपमधून अभिप्राय पाठवा, समस्या नोंदवा किंवा नवीन सुविधा सुचवा." }
    ],
    quickStart: [
      {
        title: "योग्य खाते निवडा",
        description: "काहीही जोडण्यापूर्वी खाते बदलणारा पर्याय वापरा, कारण प्रत्येक खर्च, बजेट, आयात, समूह आणि अहवाल एका खात्याशी जोडलेला असतो."
      },
      {
        title: "खर्च नोंदवा किंवा आयात करा",
        description: "दररोजचे खर्च स्वतः जोडा, किंवा स्टेटमेंट अपलोड करून फक्त जतन करायच्या ओळी मंजूर करा."
      },
      {
        title: "बजेट आणि अहवाल पहा",
        description: "मर्यादांसाठी बजेट आणि खर्चाचे नमुने समजण्यासाठी अहवाल वापरा. सामायिक समूह निपटारे तुमच्या अंतिम वैयक्तिक खर्चात दिसतात."
      }
    ],
    sections: [
      {
        title: "तुमच्या खाते कार्यक्षेत्रापासून सुरू करा",
        description: "NovaCent प्रत्येक खाते वेगळे ठेवते, म्हणून वर किंवा मोबाइल मेनूमधील खाते बदलणारा पर्याय आधी तपासा.",
        steps: [
          "खर्च, बजेट, समूह किंवा आयात जोडण्यापूर्वी काम करायचे खाते निवडा.",
          "वेगळा वाचन अनुभव हवा असल्यास भाषा आणि थीम नियंत्रण वापरा.",
          "नेव्हिगेशनच्या खाली Hi <नाव> उघडून प्रोफाइल पहा, नाव अपडेट करा किंवा पासवर्ड बदला.",
          "ऑनलाइन स्थिती पहा. ऑफलाइन असताना NovaCent समर्थित बदल स्थानिक पातळीवर ठेवते आणि ऑनलाइन झाल्यावर समन्वय करते.",
          "धूसर प्रगती स्क्रीन दिसल्यास, पुढे जाण्यापूर्वी NovaCent सेव, सिंक किंवा प्रोसेस पूर्ण होऊ द्या."
        ]
      },
      {
        title: "दररोजचे खर्च जोडा",
        description: "खर्च पान जलद मॅन्युअल नोंदी आणि खर्च वहीची समीक्षा यासाठी आहे.",
        steps: [
          "खर्च पान उघडा आणि वरचा जलद जोडा पर्याय वापरा.",
          "व्यापारी, श्रेणी, रक्कम, चलन आणि तारीख भरा. तारीख आपोआप आजची असते.",
          "जुन्या नोंदी व्यापारी किंवा श्रेणीने शोधण्यासाठी शोध वापरा.",
          "सर्व व्यवहार आणि फक्त खरे खर्च यांच्यात बदलण्यासाठी दृश्य फिल्टर वापरा.",
          "एखादी ओळ चुकीची वर्गीकृत असेल तर ओळीच्या ड्रॉपडाउनमधून श्रेणी किंवा मनी-फ्लो प्रकार बदला.",
          "चेकबॉक्सने खर्च निवडा आणि निवडलेले अनलॉक खर्च एका पुष्टी केलेल्या कृतीत हटवा.",
          "लॉक नसलेले खर्च पुष्टीकरणानंतरच हटवा. निपटारा ओळी आणि निपटलेले समूह खर्च सुरक्षित राहतात."
        ]
      },
      {
        title: "गुंतवणूक खर्चापासून वेगळी ठेवा",
        description: "गुंतवणूक रोजच्या खर्चापासून वेगळी ट्रॅक होते, त्यामुळे बजेट आणि एकूण रक्कम उपयोगी राहते.",
        steps: [
          "स्टॉक्स, म्युच्युअल फंड, विमा गुंतवणूक, पोस्टल गुंतवणूक किंवा इतर गुंतवणूक असलेल्या ओळी पाहण्यासाठी गुंतवणूक उघडा.",
          "यादी कमी करण्यासाठी 1 महिना, 3 महिने, 1 वर्ष, 3 वर्षे, सर्व किंवा स्वतःची तारीख श्रेणी वापरा.",
          "निवडलेल्या कालावधीतील गुंतवणूक रक्कम पाहण्यासाठी फिल्टर केलेला एकूण पहा.",
          "स्टेटमेंट ओळ चुकीची टॅग झाली असल्यास लेजर ड्रॉपडाउनमधून श्रेणी किंवा मनी-फ्लो प्रकार बदला."
        ]
      },
      {
        title: "बजेटवर लक्ष ठेवा",
        description: "बजेटमुळे महिना किंवा वर्ष अजून नियंत्रणात आहे का ते दिसते.",
        steps: [
          "मासिक किंवा वार्षिक बजेट तयार करा आणि ते एकूण खर्चावर लक्ष ठेवेल की एका श्रेणीवर ते निवडा.",
          "मासिक बजेट चालू महिन्याच्या 1 तारखेपासून महिन्याच्या शेवटपर्यंत चालते. वार्षिक बजेट 1 जानेवारी ते 31 डिसेंबर चालते.",
          "गणनेत कोणते खर्च घेतले आहेत हे पाहण्यासाठी बजेट उघडून पहा.",
          "स्वतःची चेतावणी मर्यादा हवी असल्यास अलर्ट टक्केवारी सेट करा. डीफॉल्ट 80% आहे."
        ]
      },
      {
        title: "बँक स्टेटमेंट आयात करा",
        description: "स्टेटमेंट ओळी खर्च वहीत जतन करण्यापूर्वी NovaCent ने वाचाव्यात यासाठी आयात समीक्षा वापरा.",
        steps: [
          "समर्थित स्टेटमेंट फाइल अपलोड करा. PDF लॉक असेल तर अपलोडपूर्वी स्टेटमेंट पासवर्ड टाका.",
          "मंजूर करण्यापूर्वी काढलेल्या ओळींची समीक्षा करा. मंजूर केल्याशिवाय काहीही खर्च म्हणून जतन होत नाही.",
          "ड्रॉपडाउनमधून श्रेणी निवडा किंवा ओळ अवर्गीकृत ठेवा.",
          "दुसऱ्या स्टेटमेंटमध्ये आधी आयात केलेल्या ओळी असू शकतात तर संभाव्य पुनरावृत्ती फिल्टर वापरा.",
          "मान्य ओळी एकेक करून मंजूर करा, सर्व सुरक्षित ओळी मंजूर करा, संभाव्य पुनरावृत्ती सोडून मंजूर करा, किंवा आयात चुकीचा असेल तर सर्व तयार केलेल्या ओळी हटवा."
        ]
      },
      {
        title: "समूहांमध्ये खर्च सामायिक करा",
        description: "समूह नोंदणीकृत मित्र किंवा नोंदणी नसलेल्या तात्पुरत्या सहभागींसोबत सामायिक खर्चासाठी आहेत.",
        steps: [
          "स्पष्ट नावाने समूह तयार करा आणि उघडा.",
          "नाव किंवा ईमेलने मित्र शोधा. नोंदणीकृत वापरकर्ता नसेल तर टाकलेले नाव नोंदणी नसलेली व्यक्ती म्हणून जोडा.",
          "समूह खर्च जोडा आणि समान, मॅन्युअल रक्कम, टक्केवारी किंवा हिस्स्यावर आधारित वाटणी निवडा.",
          "नोंदणीकृत सहभागी निपटारा मंजुरीची विनंती करतात. नोंदणी नसलेले तात्पुरते सहभागी थेट निपटतात.",
          "बदललेले समूह खर्च अंतिम वैयक्तिक खर्चानुसार एकूण खर्च वहीवर परिणाम करतात."
        ]
      },
      {
        title: "आवर्ती खर्च स्वयंचलित करा",
        description: "आवर्ती नियम बिलांसाठी किंवा पार्श्वभूमीत आपोआप नोंद होणाऱ्या खर्चांसाठी आहेत.",
        steps: [
          "व्यापारी, श्रेणी, रक्कम, चलन, वारंवारता, सुरू तारीख आणि ऐच्छिक शेवटची तारीख यांसह नियम तयार करा.",
          "खरा बिल बदलल्यास नियम थांबवा, पुन्हा सुरू करा, संपादित करा किंवा समाप्त करा.",
          "नोंद झाल्यावर आवर्ती खर्च सामान्य खर्च वही, बजेट आणि अहवालात येतात."
        ]
      },
      {
        title: "अहवाल वाचा",
        description: "अहवाल खर्च वहीला खर्च, रोख प्रवाह, बजेट फरक, व्यापारी, समूह आणि चलनांचे चार्टमध्ये बदलतात.",
        steps: [
          "मागील 15 दिवस, 1 महिना, 3 महिने, 1 वर्ष, 3 वर्षे, सर्व किंवा स्वतःची तारीख श्रेणी निवडण्यासाठी कालावधी निवडणारा पर्याय वापरा.",
          "पैसे कुठे जात आहेत हे समजण्यासाठी श्रेणीनुसार खर्च पहा.",
          "गुंतवणूक क्रिया सामान्य खर्चापासून वेगळी पाहण्यासाठी गुंतवणूक अहवाल वापरा.",
          "उत्पन्न, खर्च आणि बचत तुलना करण्यासाठी रोख प्रवाहाचा कल वापरा.",
          "स्प्रेडशीटसाठी CSV निर्यात करा किंवा छापता येणाऱ्या अहवालासाठी PDF निर्यात करा."
        ]
      },
      {
        title: "सूचना आणि सहाय्य पहा",
        description: "सूचना बजेट अलर्ट, आयात, निपटारा विनंत्या आणि महत्त्वाचे अपडेट एकत्र ठेवतात.",
        steps: [
          "न वाचलेल्या सूचना पाहण्यासाठी घंटी चिन्ह उघडा.",
          "समीक्षा झाल्यावर वाचलेले मार्क करा वापरा, किंवा जुन्या सूचना काढण्यासाठी साफ करा वापरा.",
          "सुविधा विनंती, समस्या अहवाल किंवा कौतुक पाठवण्यासाठी सहाय्य वापरा."
        ]
      }
    ]
  }
};

export type DictionaryKey = keyof typeof dictionary.en;
